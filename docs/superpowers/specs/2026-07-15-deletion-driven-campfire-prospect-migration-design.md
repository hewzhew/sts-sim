...
During migration, each upgrade candidate also carries the stable `CombatCard.uuid` beside its display-only deck index. `UpgradePlanV1.best_smith_debt_paid` is the sole durable source for the plan-level best debt; the legacy Rest-versus-Smith structure may temporarily mirror that value only for the legacy Campfire reader and may not gain new consumers. Shop and random-upgrade readers move to the plan-level fact before cutover. A Campfire Smith growth record binds by UUID, verifies the exact projected upgrade count, and copies the planner's typed mechanical delta, roles, debts, urgency, and verdict without copying a scalar score. The immediate transition is exact, but the aggregate growth field remains `Partial` until downstream outcome value is represented. Non-Smith growth remains `Unsupported` in this slice rather than receiving fabricated values.
