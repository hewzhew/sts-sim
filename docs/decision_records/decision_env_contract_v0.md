...
- `src/app/policy_runner` now defines `NeutralProbeEvaluator`, a neutral evidence/audit component rather than a policy. It builds uniform no-model proposals, requests neutral branch-compression evidence, and emits `short_horizon_signal_candidate_id` only as a diagnostic search signal.
- live CommunicationMod combat writes a `search_aware_policy_trace` from `NeutralProbeEvaluator` into combat audit. Current live execution still uses the existing baseline command path for safety; the neutral trace is shadow evidence, not takeover authority and never emits `selected_action_id`.
