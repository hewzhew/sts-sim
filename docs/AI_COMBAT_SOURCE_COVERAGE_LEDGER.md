...
| `vfx/PlayerTurnEffect.java` | mechanical_hosted_in_ui | recharge player energy, fire relic/power `onEnergyRecharge`, reveal monster intents, then fire power `atEnergyGain` when the banner finishes | combat lifecycle mechanic hidden in turn banner VFX; Rust has an explicit player-turn energy recharge hook for implemented powers such as `Energized`; future `Collect`/`Deva`/`atEnergyGain` support must attach here, not to generic `at_turn_start`; ignore text, sound, fade, and duration |
