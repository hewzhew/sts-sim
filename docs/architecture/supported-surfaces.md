...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `sts_simulator` core library | `src/lib.rs` | Owns game content, state transitions, simulation, and stable lower policy layers. | `sts_oracle_runtime`, core tests, and downstream simulator users. | Domain state and transition contracts; the crate root performs no IO. | Runtime/search layers, core tests, and downstream code. | The runtime is an upper layer, not a replacement. | Cargo workspace metadata plus public exports in `src/lib.rs`. | `SupportedMainline` | Keep the dependency direction `runtime -> core`. |
| `sts_oracle_runtime` library | `crates/sts_oracle_runtime/src/lib.rs` with source modules under `src/ai`, `src/eval`, and `src/runtime/branch` | Owns combat search, evaluation, run-control, and branch orchestration. | Oracle command hosts, runtime tests, repository tools, and diagnostics. | Run capsules, panels, combat cases, laboratory artifacts, datasets, and other typed JSON/JSONL contracts. | Maintained binaries, tools, tests, and human diagnostics. | Replaced the former duplicate control implementation; it is the sole runtime semantic owner. | Cargo workspace metadata, runtime library tests, and current command consumers. | `SupportedMainline` | Keep runtime semantics independent of command-host IO. |
...
| Package build scripts | `build.rs` and `crates/sts_oracle_runtime/build.rs` | Core generates enum-name adapters from the compiled protocol schema; runtime exports build identity and the imitation-build contract. | Cargo automatically. | Core writes `$OUT_DIR/generated_schema.rs`; runtime writes only Cargo build metadata. | Core fixtures and runtime artifacts. | The command-tools package deliberately has no build script. | Direct writer/reader trace and architecture checks. | `SupportedMainline` | Keep build identity owned by the crate that consumes it and watcher inputs narrow. |
