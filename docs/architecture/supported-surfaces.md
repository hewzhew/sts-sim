...
| `sts_oracle_eval` library | `crates/sts_oracle_eval/src/lib.rs` with the single source owner under `src/eval` | Owns optimized combat evaluation, exact-search orchestration, run-control, and shared source/artifact contracts. | Branch runtime, oracle command hosts through the runtime re-export, tests, and exact diagnostics. | Combat cases, laboratory artifacts, run journals, search progress, and other typed evaluation contracts. | Branch runtime, maintained binaries, tools, tests, and humans. | Extracted from the former combined O2 runtime so evaluation source compiles once and has an independent invalidation boundary. | Cargo dependency direction, eval/runtime tests, architecture checks, and measured warm rebuild timings. | `SupportedMainline` | Keep eval independent of `runtime::branch`; place only genuinely hot evaluation/run-control work here. |
| `sts_oracle_runtime` library | `crates/sts_oracle_runtime/src/lib.rs` with its source owner under `src/runtime/branch` | Owns branch execution, scheduling, persistence, panel/capsule projection, and resident service orchestration. | Oracle command hosts, runtime tests, repository tools, and diagnostics. | Run capsules, panels, service responses, continuation state, and branch-level typed artifacts. | Maintained binaries, tools, tests, and human diagnostics. | Consumes and re-exports `sts_oracle_eval` without recompiling its source; no evaluation implementation is duplicated. | Cargo workspace metadata, runtime library tests, current command consumers, and the matched production panel. | `SupportedMainline` | Keep branch orchestration independent of command-host IO and cheap to rebuild. |
| `sts_oracle_tools` command package | `crates/sts_oracle_tools/Cargo.toml` with binary sources under `src/bin` | Owns six maintained CLI adapters and four cross-layer integration contracts; it has no library target. | Repository scripts, runbook commands, tests, and humans. | Only the artifacts explicitly written through runtime APIs by each command. | Humans and offline tools. | Replaces the retired `sts_simulator_control` compatibility facade without duplicating or reexporting runtime semantics. | Cargo target metadata, current runbook commands, and direct dependency on `sts_oracle_runtime`. | `SupportedMainline` | Keep adapters thin and move reusable behavior into the runtime. |
...
Cargo metadata returns core, evaluation, branch runtime, and command hosts as distinct production
packages. The root `build.rs` reads only `tools/compiled_protocol_schema.json` and emits
