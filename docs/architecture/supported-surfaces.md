...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `sts_simulator` core library | `src/lib.rs` | Owns game content, state transitions, simulation, and stable lower policy layers. | `sts_oracle_eval`, core tests, and downstream simulator users. | Domain state and transition contracts; the crate root performs no IO. | Evaluation/search layers, core tests, and downstream code. | Oracle evaluation is an upper layer, not a replacement. | Cargo workspace metadata plus public exports in `src/lib.rs`. | `SupportedMainline` | Keep the dependency direction `eval -> core`. |
| `sts_oracle_eval` library | `crates/sts_oracle_eval/src/lib.rs` and `src/eval/mod.rs` | Owns optimized combat evaluation, exact-search orchestration, and shared combat artifact contracts. | Combat-search worker, run-control, tests, and exact diagnostics. | Search benchmarks, captures, combat laboratories, and typed combat evidence. | Run-control, maintained diagnostics, tests, and humans. | Replaces the former 82k-line combined compilation unit with a lower combat owner. | Cargo dependency direction, owner tests, architecture checks, and depfiles. | `SupportedMainline` | Never import run-control, learning, branch runtime, or command hosts. |
| `sts_oracle_run_control` library | `crates/sts_oracle_run_control` attaching its owned modules under `src/eval/run_control` plus run-bound case context | Owns exact run sessions, non-combat decisions, combat application, run evidence, and policy priors. | Learning adapters, branch runtime, tests, and run diagnostics. | Combat cases, run journals, search progress, and typed decision evidence. | Learning, runtime, tools, tests, and humans. | Split downstream from combat eval so run-control edits cannot invalidate the combat owner. | Cargo dependency direction, owner tests, architecture checks, and depfiles. | `SupportedMainline` | Continue separating downstream workspace/session and policy domains; do not recreate an eval facade. |
| `sts_oracle_learning_env` library | `crates/sts_oracle_learning_env` attaching the single-episode learning and root-artifact modules under `src/eval/run_control` | Owns exact run/combat learning episodes and opaque combat-root artifacts. | `sts_oracle_learning`. | Versioned combat-root batches and typed in-process episode boundaries. | Batched learning adapters and their tests. | Optimized downstream owner over run-control; exposes only the typed atomic prepare/apply seam required by pools. | Owner tests, architecture checks, and depfiles. | `SupportedMainline` | Keep model projection and Python ABI dependencies out of this hot exact-transition owner. |
| `sts_oracle_learning` library | `crates/sts_oracle_learning` attaching model-input and pool modules under `src/eval/run_control` | Owns batched pools and model-facing typed views over exact environments. | Python bridge and explicit oracle learning commands. | Model views remain in process. | Python bridge, training caller, offline collection tools, and tests. | Small downstream adapter owner; it wraps exact mechanics without owning reward, policy, or teacher semantics. | Owner tests, Python bridge verification, architecture checks, and depfiles. | `SupportedMainline` | Keep training objectives outside Rust and keep simulator-private state out of model views. |
| `sts_oracle_runtime` library | `crates/sts_oracle_runtime/src/lib.rs` with its source owner under `src/runtime/branch` | Owns branch execution, scheduling, persistence, panel/capsule projection, and resident service orchestration. | Oracle command hosts, runtime tests, repository tools, and diagnostics. | Run capsules, panels, service responses, continuation state, and branch-level typed artifacts. | Maintained binaries, tools, tests, and human diagnostics. | Consumes and re-exports `sts_oracle_run_control` without recompiling its source; no evaluation implementation is duplicated. | Cargo workspace metadata, runtime library tests, current command consumers, and the matched production panel. | `SupportedMainline` | Keep branch orchestration independent of command-host IO and cheap to rebuild. |
| `sts_oracle_tools` command package | `crates/sts_oracle_tools/Cargo.toml` with binary sources under `src/bin` | Owns six legacy CLI adapters and four cross-layer integration contracts; it has no library target. | Repository scripts, runbook commands, tests, and humans. | Only the artifacts explicitly written through runtime APIs by each command. | Humans and offline tools. | Replaces the retired `sts_simulator_control` compatibility facade; its combat-search target is now only a compatibility adapter over the dedicated owner. | Cargo target metadata, current runbook commands, and direct dependencies on `sts_oracle_runtime` and `sts_combat_search_driver`. | `SupportedMainline` | Keep adapters thin; new capability-specific frontends must not return to this broad package. |
| `sts_combat_search_driver` | `crates/sts_combat_search_driver` and `cs.cmd` | Owns one dependency-light CLI/help frontend and one optimized combat-search worker. The frontend has no simulator dependency; the worker depends directly on combat eval. | `cargo combat-search`, `cs.cmd`, ML batch scripts, the old compatibility target, tests, and humans. | The worker preserves the existing stdout/`--output` reports and laboratory artifacts; the frontend writes nothing. | ML scripts, benchmark/guidance analysis, offline labs, and humans. | Replaces both `cargo run ... combat_search_v2_driver -- --help` and the rejected attempt to embed search in the 37 MB oracle host. | Cargo graph, depfiles, frontend tests, exact-case parity smoke, and measured help/build timings. | `SupportedDiagnostic` | Keep CLI schema backend-free and keep run-control absent by package ownership. |
| Python learning bridge | `bindings/python_learning` standalone Maturin crate | Proves batched NumPy control exchange over `LearningEnvPoolV1` without per-step JSON or one call per environment slot. | Its maintained Python smoke and future model-specific bridge work. | Generated wheel, isolated verification environment, and logs below ignored `.oracle-lab/python-learning-bridge/`. | Python 3.12 smoke only; no training consumer yet. | Excluded from the root workspace so PyO3/NumPy build dependencies do not affect simulator checks; it wraps the typed pool rather than replacing it. | `verify.ps1`, exact 5-slot/330-step smoke, architecture isolation check, and wheel build. | `SupportedDiagnostic` | Add semantic feature arrays only with an explicit model encoder contract; do not describe control arrays as trainable observations. |
| Package build scripts | `build.rs`, `crates/sts_oracle_eval/build.rs`, and `crates/sts_oracle_runtime/build.rs` | Core generates enum-name adapters; eval exports profile/repository identity plus the action/guidance contract it compiles; runtime exports only its profile/repository identity. | Cargo automatically. | Core writes `$OUT_DIR/generated_schema.rs`; eval and runtime write only Cargo build metadata. | Core fixtures, evaluation artifacts, and branch runtime artifacts. | Action/guidance source watchers were removed from packages that do not compile those modules. | Direct writer/reader trace and architecture checks. | `SupportedMainline` | Keep build identity owned by the consuming crate and watcher inputs narrow. |
...
Cargo metadata returns core, combat evaluation, run-control, learning, branch runtime, and command
hosts as distinct production packages. The root `build.rs` reads only
`tools/compiled_protocol_schema.json` and emits
`$OUT_DIR/generated_schema.rs`; the evaluation build script alone watches the action/guidance
identity inputs it compiles, while the branch runtime build script emits only profile/repository
identity values. The dependency-free architecture helper confirms boundaries such as: runtime code
...
### `sts_combat_search_driver`

The lightweight frontend accepts the complete command shape but does not depend on the simulator,
evaluation, or runtime crates. Help and syntax errors therefore finish without building a search
engine. Real requests build and launch the feature-gated worker, which accepts exactly one start
spec, combat capture/snapshot, or benchmark suite and preserves all existing comparison, guidance,
laboratory, and output contracts. Run-control and learning are absent from the worker's Cargo
dependency closure. `cs.cmd` invokes the built frontend for help and the built worker
for repeated frozen-code calls; `cargo combat-search` is the build-owning path. The old
`combat_search_v2_driver` target delegates to the same worker library for compatibility only.
