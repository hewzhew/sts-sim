...
| Monster pools and encounter selection | `dungeons/AbstractDungeon.java`, `dungeons/Exordium.java`, `dungeons/TheCity.java`, `dungeons/TheBeyond.java`, `dungeons/TheEnding.java`, `monsters/MonsterInfo.java`, room classes | `src/content/monsters/encounter_pool.rs`, `src/state/run.rs`, CLI/full-run combat entrypoints | partial | `first_strong_exclusion_tables_match_java_sources`, `encounter_lists_preserve_java_generation_invariants`, `unknown_act_does_not_fall_back_to_exordium_encounter_lists`, boss-list tests | Encounter list generation, first-strong exclusions, Act 4 fixed Shield/Spear lists, boss-list generation, and no fake JawWorm/Hexaghost fallbacks are source-checked. Remaining risk is room-lifecycle parity: Java room creation reads the queue front and removes/regenerates when leaving the room, while Rust currently consumes at combat creation; concrete `MonsterHelper` encounter instantiation coverage remains separate from private monster AI fields. |
