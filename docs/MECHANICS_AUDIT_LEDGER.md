...
| Monster AI/intent internals | `monsters/AbstractMonster.java`, concrete monster classes, `EnemyMoveInfo.java`, `CommunicationMod GameStateConverter.java` runtime exports | `src/content/monsters/*`, `src/diff/state_sync/build/monster.rs`, `src/runtime/combat.rs` | partial | `docs/audits/MONSTER_RUNTIME_TRUTH_AUDIT_2026-04-18.md`, act monster audits, focused runtime truth tests; Bronze Automaton / Bronze Orb / Book of Stabbing index reconciled after `07645cb`; Centurion / Orb Walker / Spire Growth focused tests; Bandit trio SetMoveAction-chain tests; Repulsor focused tests | Migrated stateful monsters use explicit runtime truth instead of hidden-state history inference. Centurion, Orb Walker, Spire Growth, Bandit Bear/Leader/Pointy, and Repulsor are now recorded as no-hidden-runtime monsters whose history use is Java sequence logic or Java `takeTurn()` SetMoveAction chaining. Remaining risk is systematic coverage of non-migrated or newly touched monster modules; do not re-audit rows marked Good without a reopen reason. |
