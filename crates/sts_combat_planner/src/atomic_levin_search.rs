...
impl PathRank {
    ...
    fn child(parent: Self, conditional_probability: f64) -> Self {
        ...
        Self {
            atomic_depth,
            negative_log_policy,
            levin_log_priority: (atomic_depth as f64).ln() + negative_log_policy,
        }
    }
    ...
    fn expand_node(&mut self, stepper: &dyn CombatStepper, node: Arc<ExactSearchNode>) {
            ...
            .zip(probabilities[..atomic_action_count].iter().copied())
        {
            self.push_work(AtomicLevinWork::Apply(ApplyActionWork {
                parent: node.clone(),
                input,
                rank: PathRank::child(node.rank, probability),
            }));
        }
            ...
            if family.declared_min == 1 && family.effective_max == 1 {
                ...
                for (input, member_probability) in members.into_iter().zip(member_probabilities) {
                    ...
                    self.push_work(AtomicLevinWork::Apply(ApplyActionWork {
                        parent: node.clone(),
                        input,
                        rank: PathRank::child(node.rank, family_probability * member_probability),
                    }));
                }
            } else {
                ...
                let rank = PathRank::child(node.rank, member_probability);
