...
use sts_simulator::rng::{RngPool, StsRng};
...
pub fn build_pile_from_ids(ids_key: &str, snapshot: &Value, base_uuid: u32) -> Vec<CombatCard> {
    if let Some(arr) = snapshot.get(ids_key).and_then(|v| v.as_array()) {
        ...
        // Fallback: use size fields with Defend dummies
    ...
    for (i, card_val) in hand_arr.iter().enumerate() {
        ...
        if let Some(card_id) = card_id_from_java(card_id_str) {
            ...
            if let Some(cost) = card_val["cost"].as_i64() {
                let def = sts_simulator::content::cards::get_card_definition(card_id);
...
pub fn build_combat_state(snapshot: &Value, relics_val: &Value) -> CombatState {
    ...
    let mut player = PlayerEntity {
        id: 0,
        current_hp: player_val["hp"].as_i64().unwrap_or(80) as i32,
        ...
        max_orbs: 0,
        orbs: vec![],
        stance: sts_simulator::combat::StanceId::Neutral,
        relics: vec![],
    ...
    for (i, m) in monsters_arr.iter().enumerate() {
        ...
        monsters.push(MonsterEntity {
            ...
            current_hp: m["hp"].as_i64().unwrap_or(0) as i32,
...
pub fn sync_state(cs: &mut CombatState, snapshot: &Value) {
    ...
    // Sync player
    cs.player.current_hp = player_val["hp"].as_i64().unwrap_or(cs.player.current_hp as i64) as i32;
    ...
    for (i, m_val) in monsters_arr.iter().enumerate() {
        cs.monsters[i].current_hp = m_val["hp"].as_i64().unwrap_or(0) as i32;
        ...
        cs.monsters[i].next_move_byte = m_val["move_id"].as_u64().unwrap_or(0) as u8;
        ...
        for (i, p_val) in potions_arr.iter().enumerate() {
            if i < cs.potions.len() {
                cs.potions[i] = p_val.get("id")
                    ...
                    .map(|id| sts_simulator::content::potions::Potion::new(id, 0));
            }
        }
    }
