...
fn tick_until_stable(es: &mut EngineState, cs: &mut CombatState, input: ClientInput) -> bool {
    let _is_play = matches!(&input, ClientInput::PlayCard { .. });
...
fn test_diff_all_combats() {
    ...
    for combat in combats {
        println!("\n--- Combat #{} Floor {} vs [{}] ({} actions) ---",
            combat.combat_idx, combat.floor,
        ...
        for (action_idx, action) in combat.actions.iter().enumerate() {
            ...
            let input = match action.action_type.as_str() {
                "play" => {
                    ...
                    ClientInput::PlayCard { card_index: card_idx, target }
                }
                ...
                "potion" => {
                    ...
                    if parts.len() >= 3 && parts[0] == "potion" && parts[1] == "use" {
                        ...
                        ClientInput::UsePotion { potion_index: slot, target }
                    } else {
                        total_skip += 1;
                        prev_snapshot = action.result.clone();
                        continue;
            ...
            if diffs.is_empty() {
                ...
                total_fail += 1;
                let target_str = action.target.map(|t| format!(" ->target[{}]", t)).unwrap_or_default();
                println!("  [{}] {} #{}{}: FAIL {} divergence(s)", 
                    action.action_type.to_uppercase(), card_name, action_idx + 1, target_str,
                    diffs.len());
                for d in &diffs {
    ...
    println!("\n{}", "=".repeat(60));
    println!("SUMMARY: {} actions, {} pass, {} fail, {} skip",
...
fn test_diff_rng_replay() {
    let jsonl_path = concat!(env!("CARGO_MANIFEST_DIR"), "/tools/replays/2026-03-25-22-53-56--8CIJUHKRVH37.jsonl");
    ...
    println!("Loaded {} combats from {} (protocol v{})", combats.len(), jsonl_path, replay.format_version);
...
fn test_diff_all_v2_replays() {
    ...
    let v2_count = entries.iter().filter(|e| {
        let path = e.path();
            ...
            for (action_idx, action) in combat.actions.iter().enumerate() {
                ...
                if diffs.is_empty() {
                    ...
                    if !recent_actions.is_empty() {
                        println!("  ... {} preceding actions OK, last {}:", file_pass, recent_actions.len());
                        for log in &recent_actions {
                            let t = log.target.map(|t| format!(" ->target[{}]", t)).unwrap_or_default();
                            println!("    [OK] Combat #{} [{}] {} #{}{}", 
                                log.combat_idx, log.action_type.to_uppercase(), log.card_name, log.action_idx, t);
                        }
                    }
                    let target_str = action.target.map(|t| format!(" ->target[{}]", t)).unwrap_or_default();
                    println!("  >> FAIL Combat #{} [{}] {} #{}{}", 
                        combat.combat_idx, action.action_type.to_uppercase(), card_name, action_idx + 1, target_str);
                    for d in &diffs {
                        println!("       {} : Rust={}, Java={}", d.field, d.rust_val, d.java_val);
                    }
                    early_stopped = true;
    ...
    println!("\n{}", "=".repeat(60));
    println!("GRAND TOTAL: {}/{} files pass, {} actions ({} pass, {} fail, {} skip)",
        total_pass_files, total_files, grand_total_actions, grand_total_pass, grand_total_fail, grand_total_skip);
