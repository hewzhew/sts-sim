...
from micro_jaw_worm_env import ACTION_LEN, DEFAULT_DRIVER, OBS_LEN, MinimalSpireEnv
...
def parse_args() -> Args:
    ...
    parser.add_argument(
        "--save-path",
        type=Path,
        default=ROOT / "tools" / "artifacts" / "micro_jaw_worm_ppo.pt",
    )
...
def make_envs(args: Args):
    return [MinimalSpireEnv(args.driver) for _ in range(args.num_envs)]
            ...
            for step in range(args.num_steps):
                ...
                for idx, done in enumerate(done_np):
                    if done:
                        ...
                        recent_kills.append(1.0 if final_info["killed_enemy"] else 0.0)
