...
DEFAULT_DRIVER = ROOT / "target" / "debug" / (
    "micro_two_slimes_env.exe" if os.name == "nt" else "micro_two_slimes_env"
)
OBS_LEN = 128
ACTION_LEN = 21


class MicroTwoSlimesEnv(gym.Env if gym else object):
    metadata = {"render_modes": []}

    def __init__(self, driver_path: str | os.PathLike[str] | None = None):
        if np is None:
            raise RuntimeError("numpy is required for MicroTwoSlimesEnv")
        if gym is None or spaces is None:
            raise RuntimeError("gymnasium is required for MicroTwoSlimesEnv")
