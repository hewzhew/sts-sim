...
def build_pairwise_training_matrix(
    *,
    ...
    diff_rows: list[sp.csr_matrix] = []
    labels: list[int] = []
    weights: list[float] = []
    cache: dict[tuple[str, str], sp.csr_matrix] = {}

    def option_vector(decision_id: str, option_id: str) -> sp.csr_matrix:
        key = (decision_id, option_id)
        cached = cache.get(key)
        if cached is not None:
            return cached
        row = option_lookup[key]
        vector = vectorizer.transform([macro_option_feature_dict(row)])
        cache[key] = vector
        return vector
    ...
    for edge in edge_rows:
        ...
        if (decision_id, preferred_id) not in option_lookup or (decision_id, rejected_id) not in option_lookup:
            continue
        preferred_vec = option_vector(decision_id, preferred_id)
        rejected_vec = option_vector(decision_id, rejected_id)
        diff_rows.append(preferred_vec - rejected_vec)
        labels.append(1)
        weights.append(float(edge.get("strength") or 1.0))
        diff_rows.append(rejected_vec - preferred_vec)
        labels.append(0)
        weights.append(float(edge.get("strength") or 1.0))

    if not diff_rows:
        raise SystemExit("no pairwise training rows could be built from macro counterfactual edges")
    return sp.vstack(diff_rows).tocsr(), np.asarray(labels, dtype=np.int32), np.asarray(weights, dtype=np.float32)
...
def model_score_fn(model: LogisticRegression, vectorizer: DictVectorizer):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train a pairwise macro counterfactual scorer.")
    ...
    parser.add_argument("--output-prefix", default="macro_counterfactual_pairwise")
    ...
    target_scores, positive_sets, grouped_edges = build_pairwise_targets(option_rows, edge_rows)
    ...
    vectorizer.fit(macro_option_feature_dict(row) for row in train_option_rows)
    x_train, y_train, sample_weight = build_pairwise_training_matrix(
    ...
    score_fn = model_score_fn(model, vectorizer)
    ...
    model_out = args.model_out or (args.dataset_dir / f"{args.output_prefix}_model.pkl")

    metrics = {
        "model": "macro_counterfactual_pairwise_logistic",
        ...
        "notes": [
            "trained on pairwise edge differences derived from shared combat-probe counterfactual evaluations",
            "features come from current macro state, option semantics, and option deltas only; probe outcomes are not used as model inputs",
            "decision positives are derived from aggregate pairwise margins inside each decision graph",
        ],
    }
    ...
    print(f"wrote macro counterfactual metrics to {metrics_out}")
    print(f"wrote macro counterfactual predictions to {predictions_out}")
    print(f"wrote macro counterfactual model to {model_out}")
    return 0
