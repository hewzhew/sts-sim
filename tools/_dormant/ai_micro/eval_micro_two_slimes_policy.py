...
from micro_two_slimes_env import DEFAULT_DRIVER, MicroTwoSlimesEnv
from train_micro_two_slimes_ppo import Agent


ROOT = Path(__file__).resolve().parents[2]
DEFAULT_CHECKPOINT = ROOT / "tools" / "artifacts" / "micro_two_slimes_ppo.pt"
...
def load_agent(checkpoint: Path, device: torch.device) -> Agent:
    if not checkpoint.exists():
        raise FileNotFoundError(
            f"checkpoint not found: {checkpoint}. Run train_micro_two_slimes_ppo.py first."
        )
...
def run_policy(name: str, env: MicroTwoSlimesEnv, seeds: list[int], agent: Agent | None, device):
            ...
            if terminated or truncated:
                results.append(
                    EpisodeResult(
                        ...
                        reward=total_reward,
                        steps=steps,
                        killed=bool(info["killed_all"]),
                        truncated=bool(truncated),
                        player_hp=int(info["player_hp"]),
                        enemy_hp=[int(enemy["hp"]) for enemy in info["enemies"]],
                    )
                )
                break
    return results
