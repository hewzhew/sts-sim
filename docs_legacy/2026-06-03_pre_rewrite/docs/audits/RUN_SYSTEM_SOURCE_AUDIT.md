...
2. Potion global lifetime:
   ...
   - current progress: observation now reflects Java top-panel affordances for
     ...
     a potion under Sozu.
   - current progress: combat potion `canUse` now preserves Java's override
     split. Generic potions inherit `AbstractPotion.canUse()` and are blocked
     when monsters are basically dead; Blood Potion, Fruit Juice, and Entropic
     Brew use their own `canUse()` overrides and do not inherit that alive
     monster gate.
