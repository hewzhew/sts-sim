...
use crate::combat::{
    CombatState, CombatCard, CombatPhase, PlayerEntity, MonsterEntity,
    EphemeralCounters, RelicBuses, Intent, Power,
};
use crate::content::cards::CardId;
use crate::content::relics::{RelicId, RelicState};
use crate::rng::{RngPool, StsRng};

use super::mapper::{card_id_from_java, java_potion_id_to_rust, relic_id_from_java, monster_id_from_java, intent_from_java, power_id_from_java};
...
pub fn build_pile_from_ids(ids_key: &str, snapshot: &Value, base_uuid: u32) -> Vec<CombatCard> {
    ...
    if let Some(arr) = snapshot.get(&obj_key).and_then(|v| v.as_array()) {
        ...
        for (i, card_val) in arr.iter().enumerate() {
            ...
            if let Some(card_id) = card_id_from_java(id_str) {
                let mut card = CombatCard::new(card_id, card_val["uuid"].as_u64().unwrap_or(base_uuid as u64 + i as u64) as u32);
    ...
    let size = snapshot.get(&size_key).and_then(|v| v.as_u64()).unwrap_or(0) as usize;
    (0..size).map(|i| CombatCard::new(CardId::Defend, base_uuid + i as u32)).collect()
}
...
pub fn build_hand_from_snapshot(snapshot: &Value) -> Vec<CombatCard> {
    ...
    for (i, card_val) in hand_arr.iter().enumerate() {
        ...
        if let Some(card_id) = card_id_from_java(card_id_str) {
            let mut card = CombatCard::new(card_id, card_val["uuid"].as_u64().unwrap_or(i as u64 + 1000) as u32);
            card.upgrades = card_val["upgrades"].as_u64().unwrap_or(0) as u8;
    ...
    if let Some(arr) = powers_arr.as_array() {
        for p in arr {
            if let Some(pid) = power_id_from_java(p["id"].as_str().unwrap_or("")) {
                powers.push(Power {
                    power_type: pid,
                    amount: p["amount"].as_i64().unwrap_or(0) as i32,
                    extra_data: 0,
                    just_applied: false,
                });
            }
...
pub fn build_combat_state(snapshot: &Value, relics_val: &Value) -> CombatState {
    ...
    let mut player = PlayerEntity {
        id: 0,
        current_hp: player_val["current_hp"].as_i64().unwrap_or(player_val["hp"].as_i64().unwrap_or(80)) as i32,
    ...
    let effective_relics = if snapshot.get("relics").and_then(|r| r.as_array()).map_or(false, |a| !a.is_empty()) {
        &snapshot["relics"]
    } else {
        relics_val
    };
    ...
    // Mark one-shot-per-combat relics as used_up 
    let turn = snapshot.get("turn").and_then(|v| v.as_u64()).unwrap_or(1);
    if turn >= 1 {
        for relic in player.relics.iter_mut() {
            match relic.id {
                RelicId::Lantern | RelicId::CentennialPuzzle => relic.used_up = true,
                _ => {}
            }
        }
    }
    ...
    for (i, m) in monsters_arr.iter().enumerate() {
        ...
        monsters.push(MonsterEntity {
            id: entity_id,
            monster_type: monster_id_from_java(m["id"].as_str().unwrap_or("")).map(|e| e as usize).unwrap_or(0),
            current_hp: m["current_hp"].as_i64().unwrap_or(m["hp"].as_i64().unwrap_or(0)) as i32,
            max_hp: m["max_hp"].as_i64().unwrap_or(0) as i32,
            block: m["block"].as_i64().unwrap_or(0) as i32,
            slot: i as u8,
            is_dying: m.get("is_gone").and_then(|v| v.as_bool()).unwrap_or(false),
            half_dead: m.get("half_dead").and_then(|v| v.as_bool()).unwrap_or(false),
            is_escaped: false,
    ...
    for (i, m) in monsters_arr.iter().enumerate() {
        let entity_id = i + 1;
        let powers = build_powers_from_snapshot(&m["powers"]);
        if !powers.is_empty() {
        ...
        for (i, p) in arr.iter().enumerate() {
            ...
            if pid_str != "Potion Slot" {
                if let Some(rust_id) = java_potion_id_to_rust(pid_str) {
                    parsed_potions.push(Some(crate::content::potions::Potion::new(rust_id, i as u32)));
                } else {
    ...
    if let Some(rng_state) = snapshot.get("rng_state") {
        ...
        if let Some(r) = parse_rng("ai_rng") { rng_pool.ai_rng = r; }
        if let Some(r) = parse_rng("shuffle_rng") { rng_pool.shuffle_rng = r; }
        if let Some(r) = parse_rng("card_rng") { rng_pool.card_random_rng = r; }
        if let Some(r) = parse_rng("misc_rng") { rng_pool.misc_rng = r; }
        if let Some(r) = parse_rng("monster_hp_rng") { rng_pool.monster_hp_rng = r; }
        if let Some(r) = parse_rng("potion_rng") { rng_pool.potion_rng = r; }
    }
    ...
    let mut cs = CombatState {
        ...
        rng: rng_pool,
        is_boss_fight: snapshot.get("room_type").map_or(false, |s| s.as_str() == Some("MonsterRoomBoss")),
        is_elite_fight: snapshot.get("room_type").map_or(false, |s| s.as_str() == Some("MonsterRoomElite") || s.as_str() == Some("EventRoom")),
...
pub fn sync_state(cs: &mut CombatState, snapshot: &Value) {
    ...
    // Sync player
    cs.player.current_hp = player_val["current_hp"].as_i64().unwrap_or(player_val["hp"].as_i64().unwrap_or(cs.player.current_hp as i64)) as i32;
    cs.player.max_hp = player_val["max_hp"].as_i64().unwrap_or(cs.player.max_hp as i64) as i32;
    cs.player.block = player_val["block"].as_i64().unwrap_or(0) as i32;
    cs.energy = player_val["energy"].as_u64().unwrap_or(3) as u8;
    
    // Sync player powers
    cs.power_db.insert(0, build_powers_from_snapshot(&player_val["powers"]));
    ...
    for (i, m_val) in monsters_arr.iter().enumerate() {
        cs.monsters[i].current_hp = m_val["current_hp"].as_i64().unwrap_or(m_val["hp"].as_i64().unwrap_or(0)) as i32;
        cs.monsters[i].max_hp = m_val["max_hp"].as_i64().unwrap_or(0) as i32;
        cs.monsters[i].block = m_val["block"].as_i64().unwrap_or(0) as i32;
        cs.monsters[i].is_dying = m_val.get("is_gone").and_then(|v| v.as_bool()).unwrap_or(false);
        cs.monsters[i].monster_type = monster_id_from_java(m_val["id"].as_str().unwrap_or("")).map(|e| e as usize).unwrap_or(0);
        ...
        let powers = build_powers_from_snapshot(&m_val["powers"]);
        if !powers.is_empty() {
...
pub fn sync_rng(rng: &mut RngPool, snapshot: &Value) {
    ...
    if let Some(ai) = rng_state.get("ai_rng") { sync_rng_channel(&mut rng.ai_rng, ai); }
    if let Some(shuffle) = rng_state.get("shuffle_rng") { sync_rng_channel(&mut rng.shuffle_rng, shuffle); }
    if let Some(card) = rng_state.get("card_rng") { sync_rng_channel(&mut rng.card_random_rng, card); }
    if let Some(misc) = rng_state.get("misc_rng") { sync_rng_channel(&mut rng.misc_rng, misc); }
    if let Some(monster_hp) = rng_state.get("monster_hp_rng") { sync_rng_channel(&mut rng.monster_hp_rng, monster_hp); }
    if let Some(potion) = rng_state.get("potion_rng") { sync_rng_channel(&mut rng.potion_rng, potion); }
}

fn sync_rng_channel(rng: &mut StsRng, json: &Value) {
    if let Some(s0) = json.get("seed0").and_then(|v| v.as_i64()) { rng.seed0 = s0 as u64; }
    if let Some(s1) = json.get("seed1").and_then(|v| v.as_i64()) { rng.seed1 = s1 as u64; }
    if let Some(c) = json.get("counter").and_then(|v| v.as_u64()) { rng.counter = c as u32; }
}
