...
mod tests {
    ...
    fn sample_raw_path() -> &'static Path {
        Path::new(r"d:\rust\sts_simulator\logs\raw\live_comm_raw_20260411_214738.jsonl")
    }

    fn latest_raw_with_pending_choice_path() -> &'static Path {
        Path::new(r"d:\rust\sts_simulator\logs\raw\live_comm_raw_20260412_004625.jsonl")
    }

    #[test]
    fn raw_bridge_extracts_unique_command_steps_from_real_log() {
        let replay = build_live_session_replay_from_raw_path(sample_raw_path()).expect("replay");
        assert!(!replay.steps.is_empty());

        let mut seen = HashSet::new();
        for step in &replay.steps {
            assert!(seen.insert(step.command_id));
        }
    }

    #[test]
    fn raw_bridge_binds_before_and_after_root_for_first_step() {
        let replay = build_live_session_replay_from_raw_path(sample_raw_path()).expect("replay");
        let first = replay.steps.first().expect("first step");
        assert_eq!(first.command_id, 1);
        assert_eq!(first.response_id, Some(2));
        assert!(first.before_root.get("in_game").is_some());
        assert!(first.after_root.get("game_state").is_some());
    }

    #[test]
    fn combat_view_keeps_noncombat_steps_visible_as_skipped() {
        let replay = build_live_session_replay_from_raw_path(sample_raw_path()).expect("replay");
        let combat_view = derive_combat_replay_view(&replay);
        assert_eq!(combat_view.steps.len(), replay.steps.len());
        assert!(combat_view
            .steps
            .iter()
            .any(|step| step.status == CombatReplayStepStatus::SkippedNoncombat));
    }

    #[test]
    fn combat_verification_report_balances_step_categories() {
        let replay = build_live_session_replay_from_raw_path(sample_raw_path()).expect("replay");
        let combat_view = derive_combat_replay_view(&replay);
        let report = verify_combat_replay_view(&combat_view, true).expect("report");
        assert_eq!(
            report.total_steps,
    ...
    #[test]
    fn play_step_that_opens_hand_select_is_not_treated_as_executable() {
        let replay = build_live_session_replay_from_raw_path(latest_raw_with_pending_choice_path())
            .expect("replay");
        let combat_view = derive_combat_replay_view(&replay);
        let step = combat_view
            .steps
            .iter()
            .find(|step| step.command_id == 405)
            .expect("play step opening hand select");
        assert_eq!(step.command_text, "PLAY 3");
        assert_eq!(step.screen_type.as_deref(), Some("HAND_SELECT"));
        assert_eq!(step.status, CombatReplayStepStatus::InsufficientContext);
    }
}
