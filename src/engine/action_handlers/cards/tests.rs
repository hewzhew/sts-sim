...
fn constructed_make_copy_in_hand_separates_constructor_and_effect_reality_calls() {
    ...
    crate::content::cards::apply_master_reality_to_generated_card(&mut constructed, &hand_state, 1);
    ...
    handle_make_constructed_copy_in_hand(Box::new(overflow_constructed), 1, &mut overflow_state);
    assert_eq!(
...
fn random_pool_blood_for_blood_copy_uses_java_make_copy_damage_discount() {
    ...
    let card =
        crate::content::cards::make_fresh_card_copy_for_combat(CardId::BloodForBlood, 90, &state);
...
fn make_random_card_in_hand_uses_current_player_class_pool() {
    ...
    assert!(
        !crate::content::cards::ironclad_pool_for_type(CardType::Attack).contains(&generated.id),
...
fn random_draw_pile_insert_maps_java_bottom_to_top_order() {
    ...
    let java_insert_index = state
        ...
        .card_random_rng
        .clone()
        .random(state.zones.draw_pile.len() as i32 - 1) as usize;
    ...
    for uuid in [4_u32, 3, 1] {
        if expected_candidates.is_empty() {
            ...
            let index = expected_rng
                .card_random_rng
                .random(expected_candidates.len() as i32 - 1) as usize;
...
fn use_card_done_applies_strange_spoon_to_exhaust_on_use_once_cards() {
    for expected_saved in [true, false] {
        ...
        state.rng.card_random_rng =
            crate::runtime::rng::StsRng::new(seed_with_next_card_random_boolean(expected_saved));
