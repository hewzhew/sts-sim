...
fn combat_case_review_command(case: &str) -> String {
    format!("cargo run --quiet -p sts_oracle_tools --bin combat_case_review -- --case \"{case}\" --ladder --compact")
}

fn branch_tiny_continue_command(capsule_path: &Path, wall_ms: u64) -> String {
    format!(
        "cargo run --quiet -p sts_oracle_tools --bin branch_tiny -- --continue-capsule \"{}\" --continue-slices 1 --wall-ms {}",
...
mod tests {
    ...
    #[test]
    fn result_preserves_potion_continuation_context_in_attempts_and_history() {
        let mut branch = sample_branch();
    ...
    fn continue_recommendation_uses_cargo_run_not_stale_exe() {
        ...
        assert_eq!(
            command,
            "cargo run --quiet -p sts_oracle_tools --bin branch_tiny -- --continue-capsule \"target/capsule\" --continue-slices 1 --wall-ms 60000"
        );
    ...
    fn combat_review_recommendation_uses_cargo_run_not_stale_exe() {
        ...
        assert_eq!(
            command,
            "cargo run --quiet -p sts_oracle_tools --bin combat_case_review -- --case \"target/cases/case.json\" --ladder --compact"
        );
        assert!(!command.contains("target\\debug"));
    ...
    fn primary_search_outcome_projects_profile_and_telemetry() {
        let attempt = sts_simulator::eval::run_control::CombatSearchTraceSummary {
            ...
            report_frontier_scan_us: 0,
            report_search_storage_drop_us: 0,
            rollout_us: 11,
            expansion_us: 13,
