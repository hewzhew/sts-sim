...
impl MonsterBehavior for CorruptHeart {
        ...
        match move_count % 3 {
            0 => {
                ...
                if rng.random_boolean() {
                    (1, Intent::Attack { damage: blood_dmg, hits: blood_hits })
                } else {
                    (2, Intent::Attack { damage: echo_dmg, hits: 1 })
                }
            }
            1 => {
    ...
    fn take_turn(
        ...
        match move_byte {
            3 => { // Debilitate
                actions.push(Action::ApplyPower { source: entity.id, target: 0, power_id: crate::content::powers::PowerId::Vulnerable, amount: debuff_amt });
                actions.push(Action::ApplyPower { source: entity.id, target: 0, power_id: crate::content::powers::PowerId::Weak, amount: debuff_amt });
                actions.push(Action::ApplyPower { source: entity.id, target: 0, power_id: crate::content::powers::PowerId::Frail, amount: debuff_amt });
                ...
                for s in statuses {
                    actions.push(Action::MakeTempCardInDrawPile { card_id: s, amount: 1, random_spot: true , upgraded: false });
                }
            },
            1 => { // Blood Shots
                for _ in 0..blood_hits {
                    actions.push(Action::Damage(DamageInfo {
                        source: entity.id, target: 0, base: blood_dmg, output: blood_dmg,
            ...
            2 => { // Echo Strike
                actions.push(Action::Damage(DamageInfo {
                    source: entity.id, target: 0, base: echo_dmg, output: echo_dmg,
                    damage_type: DamageType::Normal, is_modified: false,
                }));
            },
            4 => { // Buff
                ...
                if let Some(powers) = state.power_db.get(&entity.id) {
                    if let Some(str_pow) = powers.iter().find(|p| p.power_type == crate::content::powers::PowerId::Strength) {
                        if str_pow.amount < 0 {
                ...
                actions.push(Action::ApplyPower { source: entity.id, target: entity.id, power_id: crate::content::powers::PowerId::Strength, amount: additional + 2 });
                
                // Java buff cycle: 0→Artifact(2), 1→BeatOfDeath(1), 2→PainfulStabs, 3→+10Str, 4+→+50Str
                match buff_count {
                    0 => {
                        actions.push(Action::ApplyPower { source: entity.id, target: entity.id, power_id: crate::content::powers::PowerId::Artifact, amount: 2 });
                    }
                    1 => {
                        actions.push(Action::ApplyPower { source: entity.id, target: entity.id, power_id: crate::content::powers::PowerId::BeatOfDeath, amount: 1 });
                    }
                    2 => {
                        actions.push(Action::ApplyPower { source: entity.id, target: entity.id, power_id: crate::content::powers::PowerId::PainfulStabs, amount: 1 });
                    }
                    3 => {
                        actions.push(Action::ApplyPower { source: entity.id, target: entity.id, power_id: crate::content::powers::PowerId::Strength, amount: 10 });
                    }
                    _ => {
                        actions.push(Action::ApplyPower { source: entity.id, target: entity.id, power_id: crate::content::powers::PowerId::Strength, amount: 50 });
                    }
                }
            },
            _ => {},
