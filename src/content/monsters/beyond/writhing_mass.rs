...
impl MonsterBehavior for WrithingMass {
    fn roll_move(_rng: &mut crate::rng::StsRng, entity: &crate::combat::MonsterEntity, ascension_level: u8, num: i32) -> (u8, Intent) {
        ...
        if entity.move_history.is_empty() {
            if num < 33 {
                return (1, Intent::Attack { damage: multi_dmg, hits: 3 });
            } else if num < 66 {
                return (2, Intent::AttackDefend { damage: acc_dmg, hits: 1 });
            } else {
                return (3, Intent::AttackDebuff { damage: debuff_dmg, hits: 1 });
            }
        }
        if num < 10 {
            if last_move != 0 {
                return (0, Intent::Attack { damage: big_dmg, hits: 1 });
            } else {
                return (1, Intent::Attack { damage: multi_dmg, hits: 3 }); // Simplified fallback
            }
        } else if num < 20 {
            if !used_mega_debuff && last_move != 4 {
                return (4, Intent::StrongDebuff);
            } else if _rng.random_range(0, 9) == 0 { // 10% chance
                 return (0, Intent::Attack { damage: big_dmg, hits: 1 });
            } else {
                return (1, Intent::Attack { damage: multi_dmg, hits: 3 }); // Simplified fallback
            }
            ...
            if last_move != 3 {
                 return (3, Intent::AttackDebuff { damage: debuff_dmg, hits: 1 });
            } else if _rng.random_range(0, 9) < 4 { // 40% chance
                 return (2, Intent::AttackDefend { damage: acc_dmg, hits: 1 }); // fallback
            } else {
                 return (1, Intent::Attack { damage: multi_dmg, hits: 3 }); // fallback
            }
        } else if num < 70 {
             if last_move != 1 {
                 return (1, Intent::Attack { damage: multi_dmg, hits: 3 });
                 ...
                 return (3, Intent::AttackDebuff { damage: debuff_dmg, hits: 1 }); // fallback
             }
        } else if last_move != 2 {
             return (2, Intent::AttackDefend { damage: acc_dmg, hits: 1 });
        } else {
    ...
    fn use_pre_battle_action(entity: &crate::combat::MonsterEntity, _hp_rng: &mut crate::rng::StsRng, _ascension_level: u8) -> Vec<Action> {
        vec![
            ...
            Action::ApplyPower {
                ...
                amount: 1, // Represents presence usually
            }
        ]
    }
    ...
    fn take_turn(state: &mut CombatState, entity: &crate::combat::MonsterEntity) -> Vec<Action> {
        ...
        match entity.next_move_byte {
            ...
            2 => { // ATTACK_BLOCK
                 ...
                 actions.push(Action::GainBlock { target: entity.id, amount: acc_dmg });
            },
            3 => { // ATTACK_DEBUFF
                 actions.push(Action::Damage(DamageInfo {
                     source: entity.id,
            ...
            4 => { // MEGA_DEBUFF
                 actions.push(Action::AddCardToMasterDeck { card_id: CardId::Parasite });
            },
            _ => {}
        }
