...
impl MonsterBehavior for TimeEater {
    fn roll_move(_rng: &mut crate::rng::StsRng, entity: &crate::combat::MonsterEntity, ascension_level: u8, num: i32) -> (u8, Intent) {
        ...
        if num < 45 {
            if !last_two_moves(2) {
                return (2, Intent::Attack { damage: reverb_dmg, hits: 3 });
            } else {
                return (4, Intent::AttackDebuff { damage: head_slam_dmg, hits: 1 }); // Fallback assuming AI rng cascade
            }
        } else if num < 80 {
            if last_move != 4 {
                return (4, Intent::AttackDebuff { damage: head_slam_dmg, hits: 1 });
            ...
            return (4, Intent::AttackDebuff { damage: head_slam_dmg, hits: 1 }); // Simplified cascade 
        }
    }
    ...
    fn take_turn(state: &mut CombatState, entity: &crate::combat::MonsterEntity) -> Vec<Action> {
        ...
        match entity.next_move_byte {
            ...
            3 => { // RIPPLE
                actions.push(Action::GainBlock { target: entity.id, amount: 20 });
            ...
            5 => { // HASTE
                 ...
                 if asc >= 19 {
                     actions.push(Action::GainBlock { target: entity.id, amount: head_slam_dmg }); // head_slam_dmg used as block in Java
                 }
            },
            _ => {}
        }
