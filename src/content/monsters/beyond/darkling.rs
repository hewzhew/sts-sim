...
impl MonsterBehavior for Darkling {
    fn roll_move(_rng: &mut crate::rng::StsRng, entity: &crate::combat::MonsterEntity, ascension_level: u8, num: i32) -> (u8, Intent) {
        ...
        let nip_dmg = _rng.random_range(if ascension_level >= 2 { 9 } else { 7 }, if ascension_level >= 2 { 13 } else { 11 }) as i32;
        ...
        if entity.move_history.is_empty() {
            if num < 50 {
                return (2, if ascension_level >= 17 { Intent::DefendBuff } else { Intent::Defend });
            } else {
                return (3, Intent::Attack { damage: nip_dmg, hits: 1 });
            }
        }
        ...
        if num < 40 {
            if last_move != 1 {
                (1, Intent::Attack { damage: chomp_dmg, hits: 2 })
            } else {
                (3, Intent::Attack { damage: nip_dmg, hits: 1 }) // Simplified fallback from deep ai_rng nested
            }
        } else if num < 70 {
            if last_move != 2 {
                (2, if ascension_level >= 17 { Intent::DefendBuff } else { Intent::Defend })
            } else {
                (3, Intent::Attack { damage: nip_dmg, hits: 1 })
            }
        } else if !last_two_moves(3) {
            (3, Intent::Attack { damage: nip_dmg, hits: 1 })
        } else {
            (2, if ascension_level >= 17 { Intent::DefendBuff } else { Intent::Defend }) // Simplified fallback
        }
    }
    ...
    fn take_turn(state: &mut CombatState, entity: &crate::combat::MonsterEntity) -> Vec<Action> {
        ...
        match entity.next_move_byte {
            ...
            4 => { // REINCARNATE
                 actions.push(Action::Heal { target: entity.id, amount: entity.max_hp / 2 });
            },
            _ => {}
        }
