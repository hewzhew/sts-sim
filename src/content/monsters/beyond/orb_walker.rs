...
impl MonsterBehavior for OrbWalker {
    fn roll_move(_rng: &mut crate::rng::StsRng, entity: &crate::combat::MonsterEntity, ascension_level: u8, num: i32) -> (u8, Intent) {
        ...
        if num < 40 {
            if !last_two_moves(2) {
                (2, Intent::Attack { damage: claw_dmg, hits: 1 })
            } else {
                (1, Intent::AttackDebuff { damage: laser_dmg, hits: 1 })
            }
    ...
    fn take_turn(state: &mut CombatState, entity: &crate::combat::MonsterEntity) -> Vec<Action> {
        ...
        match entity.next_move_byte {
            ...
            1 => { // LASER
                ...
                actions.push(Action::MakeTempCardInDiscard { card_id: CardId::Burn, amount: 1 , upgraded: false });
                actions.push(Action::MakeTempCardInDrawPile { card_id: CardId::Burn, amount: 1, random_spot: true , upgraded: false });
            },
            _ => {}
        }
