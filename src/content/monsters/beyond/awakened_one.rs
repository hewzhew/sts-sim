...
impl MonsterBehavior for AwakenedOne {
    fn roll_move(_rng: &mut crate::rng::StsRng, entity: &crate::combat::MonsterEntity, _ascension_level: u8, num: i32) -> (u8, Intent) {
        ...
        if is_phase_one {
             ...
             if num < 25 {
                  if last_move != 2 {
                       return (2, Intent::Attack { damage: soul_strike_dmg, hits: 4 });
                  } else {
                       return (1, Intent::Attack { damage: slash_dmg, hits: 1 });
                  }
             ...
             let last_move = entity.move_history.back().copied().unwrap_or(0);
             if last_move == 3 {
                  return (5, Intent::Attack { damage: dark_echo_dmg, hits: 1 });
             }
             let last_two_moves = |byte| {
                 entity.move_history.len() >= 2 && 
                 entity.move_history[entity.move_history.len()-1] == byte && 
             ...
             if num < 50 {
                  if !last_two_moves(6) {
                       return (6, Intent::AttackDebuff { damage: sludge_dmg, hits: 1 });
                  } else {
                       return (8, Intent::Attack { damage: tackle_dmg, hits: 3 });
                  }
    ...
    fn take_turn(state: &mut CombatState, entity: &crate::combat::MonsterEntity) -> Vec<Action> {
        ...
        match entity.next_move_byte {
            ...
            3 => { // REBIRTH TRIGGER
                 ...
                 actions.push(Action::Heal { target: entity.id, amount: heal_amt });
                 actions.push(Action::RemovePower { target: entity.id, power_id: PowerId::Unawakened });
            ...
            6 => { // SLUDGE
                 ...
                 actions.push(Action::MakeTempCardInDrawPile { card_id: crate::content::cards::CardId::Void, amount: 1, random_spot: true , upgraded: false });
            },
            8 => { // TACKLE
                 for _ in 0..3 {
                     actions.push(Action::Damage(DamageInfo {
