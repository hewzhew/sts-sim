...
impl MonsterBehavior for Reptomancer {
    fn roll_move(_rng: &mut crate::rng::StsRng, entity: &crate::combat::MonsterEntity, ascension_level: u8, num: i32) -> (u8, Intent) {
        ...
        if num < 33 {
            if last_move != 1 {
                 return (1, Intent::AttackDebuff { damage: scythe_dmg, hits: 2 });
            } else {
                 return (3, Intent::Attack { damage: big_bite_dmg, hits: 1 }); // fallback
            }
        } else if num < 66 {
            if !last_two_moves(2) {
                 return (2, Intent::Unknown); // SPAWN_DAGGER
             ...
             return (3, Intent::Attack { damage: big_bite_dmg, hits: 1 });
        } else {
             return (1, Intent::AttackDebuff { damage: scythe_dmg, hits: 2 }); // fallback
        }
    }

    fn use_pre_battle_action(_entity: &crate::combat::MonsterEntity, _hp_rng: &mut crate::rng::StsRng, _ascension_level: u8) -> Vec<Action> {
    ...
    fn take_turn(state: &mut CombatState, entity: &crate::combat::MonsterEntity) -> Vec<Action> {
        ...
        match entity.next_move_byte {
            ...
            2 => { // SPAWN_DAGGER
                 ...
                 let summon_count = if alive_count <= 3 { std::cmp::min(daggers_per_spawn, 4 - alive_count) } else { 0 };
