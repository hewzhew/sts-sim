...
impl MonsterBehavior for BronzeOrb {
    ...
    fn take_turn(state: &mut CombatState, entity: &crate::combat::MonsterEntity) -> Vec<Action> {
        ...
        match entity.next_move_byte {
            1 => {
                actions.push(Action::Damage(DamageInfo { source: entity.id, target: 0, base: 8, output: 8, damage_type: DamageType::Normal, is_modified: false }));
                // Let's assume the Bronze Automaton is present and we can just add the block to it.
                if let Some(automaton) = state.monsters.iter().find(|m| crate::content::monsters::EnemyId::from_id(m.monster_type) == Some(EnemyId::BronzeAutomaton) && !m.is_dying) {
                    actions.push(Action::GainBlock { target: automaton.id, amount: 12 });
                }
            },
            2 => {
                if let Some(automaton) = state.monsters.iter().find(|m| crate::content::monsters::EnemyId::from_id(m.monster_type) == Some(EnemyId::BronzeAutomaton) && !m.is_dying) {
            ...
            3 => {
                actions.push(Action::ApplyStasis { target_id: entity.id });
            },
            _ => {}
        }
