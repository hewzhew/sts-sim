...
impl MonsterBehavior for Mugger {
    fn use_pre_battle_action(entity: &MonsterEntity, _rng: &mut crate::rng::StsRng, ascension_level: u8) -> Vec<Action> {
    ...
    fn roll_move(_rng: &mut crate::rng::StsRng, _entity: &MonsterEntity, ascension_level: u8, _num: i32) -> (u8, Intent) {
        ...
        (1, Intent::Attack { damage: swipe_dmg, hits: 1 })
    }
    ...
    fn take_turn(state: &mut CombatState, entity: &MonsterEntity) -> Vec<Action> {
        ...
        let prior_slashes = entity.move_history.iter().filter(|&&m| m == 1 || m == 4).count();
                ...
                if next_slash_count == 2 {
                    if state.rng.ai_rng.random_boolean() {
                        ...
                        actions.push(Action::SetMonsterMove {
                            ...
                            intent: Intent::Attack { damage: big_swipe_dmg, hits: 1 },
                        });
                    }
                } else {
                    actions.push(Action::SetMonsterMove {
                        ...
                        intent: Intent::Attack { damage: swipe_dmg, hits: 1 },
                    });
                }
            }
            4 => { // BIG SWIPE
