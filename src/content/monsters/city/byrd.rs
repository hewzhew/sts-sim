...
impl MonsterBehavior for Byrd {
    ...
    fn roll_move(rng: &mut crate::rng::StsRng, entity: &MonsterEntity, ascension_level: u8, num: i32) -> (u8, Intent) {
        ...
        if entity.move_history.is_empty() {
            if rng.random_boolean_chance(0.375) {
                ...
                return (1, Intent::Attack { damage: peck_dmg, hits: peck_count as u8 });
            }
        }
        ...
        if is_flying {
            if num < 50 {
                ...
                if rev.next() == Some(&1) && rev.next() == Some(&1) {
                    if rng.random_boolean_chance(0.4) {
                        (3, Intent::Attack { damage: swoop_dmg, hits: 1 })
                    } else {
                        (6, Intent::Buff)
                    }
                } else {
                    (1, Intent::Attack { damage: peck_dmg, hits: peck_count as u8 })
                }
            } else if num < 70 {
            ...
            (5, Intent::Attack { damage: headbutt_dmg, hits: 1 })
        }
    }
    ...
    fn take_turn(state: &mut CombatState, entity: &MonsterEntity) -> Vec<Action> {
        ...
        match entity.next_move_byte {
            ...
            2 => { // Go Airborne (Not typically used during combat via roll_move, but in case)
