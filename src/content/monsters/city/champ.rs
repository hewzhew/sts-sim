...
impl MonsterBehavior for Champ {
    fn roll_move(_rng: &mut crate::rng::StsRng, entity: &crate::combat::MonsterEntity, ascension_level: u8, num: i32) -> (u8, Intent) {
        ...
        if threshold_reached {
            if !last_move(3) && !last_move_before(3) {
                return (3, Intent::Attack { damage: 10, hits: 2 }); // EXECUTE
            }
        }
        ...
        if !last_move(4) && num <= 55 {
            return (4, Intent::AttackDebuff { damage: slap_dmg, hits: 1 }); // FACE_SLAP
        }

        let slash_dmg = if ascension_level >= 4 { 18 } else { 16 };
        if !last_move(1) {
            (1, Intent::Attack { damage: slash_dmg, hits: 1 }) // HEAVY_SLASH
        } else {
            (4, Intent::AttackDebuff { damage: slap_dmg, hits: 1 }) // FACE_SLAP
        }
    }
    ...
    fn take_turn(state: &mut CombatState, entity: &crate::combat::MonsterEntity) -> Vec<Action> {
        ...
        let block_amt = if ascension_level >= 19 { 20 } else if ascension_level >= 9 { 18 } else { 15 };
        let forge_amt = if ascension_level >= 19 { 7 } else if ascension_level >= 9 { 6 } else { 5 };
        let str_amt = if ascension_level >= 19 { 4 } else if ascension_level >= 4 { 3 } else { 2 };
        ...
        match entity.next_move_byte {
            1 => { // HEAVY_SLASH
                actions.push(Action::Damage(DamageInfo { source: entity.id, target: 0, base: slash_dmg, output: slash_dmg, damage_type: DamageType::Normal, is_modified: false }));
            },
            2 => { // DEFENSIVE_STANCE
                actions.push(Action::GainBlock { target: entity.id, amount: block_amt });
                actions.push(Action::ApplyPower { source: entity.id, target: entity.id, power_id: PowerId::Metallicize, amount: forge_amt });
            },
            3 => { // EXECUTE
                for _ in 0..2 {
                    actions.push(Action::Damage(DamageInfo { source: entity.id, target: 0, base: 10, output: 10, damage_type: DamageType::Normal, is_modified: false }));
                }
            },
            4 => { // FACE_SLAP
                actions.push(Action::Damage(DamageInfo { source: entity.id, target: 0, base: slap_dmg, output: slap_dmg, damage_type: DamageType::Normal, is_modified: false }));
                actions.push(Action::ApplyPower { source: entity.id, target: 0, power_id: PowerId::Frail, amount: 2 });
                actions.push(Action::ApplyPower { source: entity.id, target: 0, power_id: PowerId::Vulnerable, amount: 2 });
            },
            5 => { // GLOAT
                actions.push(Action::ApplyPower { source: entity.id, target: entity.id, power_id: PowerId::Strength, amount: str_amt });
            },
            6 => { // TAUNT
                actions.push(Action::ApplyPower { source: entity.id, target: 0, power_id: PowerId::Weak, amount: 2 });
                actions.push(Action::ApplyPower { source: entity.id, target: 0, power_id: PowerId::Vulnerable, amount: 2 });
            },
            7 => { // ANGER
                actions.push(Action::RemoveAllDebuffs { target: entity.id });
                
                actions.push(Action::ApplyPower { source: entity.id, target: entity.id, power_id: PowerId::Strength, amount: str_amt * 3 });
            },
            _ => {}
        }
