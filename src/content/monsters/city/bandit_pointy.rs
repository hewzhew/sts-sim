...
impl MonsterBehavior for BanditPointy {
    fn roll_move(_rng: &mut crate::rng::StsRng, _entity: &crate::combat::MonsterEntity, ascension_level: u8, _num: i32) -> (u8, Intent) {
        let atk_dmg = if ascension_level >= 2 { 6 } else { 5 };
        (1, Intent::Attack { damage: atk_dmg, hits: 2 }) // POINTY_SPECIAL
    }
