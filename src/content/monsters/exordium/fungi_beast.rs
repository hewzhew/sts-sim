...
impl MonsterBehavior for FungiBeast {
    fn roll_move(_rng: &mut crate::rng::StsRng, entity: &MonsterEntity, ascension_level: u8, num: i32) -> (u8, Intent) {
        let bite_dmg = 6;
        let _str_amt = if ascension_level >= 17 { 5 } else if ascension_level >= 2 { 4 } else { 3 };
        ...
        if num < 60 {
            if last_two_moves_were(1) {
                ...
                (1, Intent::Attack { damage: bite_dmg, hits: 1 })
            }
    ...
    fn take_turn(state: &mut CombatState, entity: &MonsterEntity) -> Vec<Action> {
        let bite_dmg = 6;
        let str_amt = if state.ascension_level >= 17 { 5 } else if state.ascension_level >= 2 { 4 } else { 3 };
