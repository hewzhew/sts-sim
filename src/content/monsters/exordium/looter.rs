...
impl MonsterBehavior for Looter {
    ...
    fn roll_move(rng: &mut crate::rng::StsRng, entity: &MonsterEntity, ascension_level: u8, _num: i32) -> (u8, Intent) {
        let slash_count = entity.move_history.iter().filter(|&&b| b == 1 || b == 4).count();
        ...
        match slash_count {
            0 => (1, Intent::Attack { damage: mug_dmg, hits: 1 }),
            1 => (1, Intent::Attack { damage: mug_dmg, hits: 1 }),
            2 => {
                if last_move == Some(2) || last_move == Some(3) {
                    (3, Intent::Escape)
                } else if rng.random_boolean() {
                    (4, Intent::Attack { damage: lunge_dmg, hits: 1 })
                } else {
                    (2, Intent::Defend)
                }
            }
