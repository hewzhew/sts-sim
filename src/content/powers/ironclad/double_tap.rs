...
pub fn on_use_card(state: &mut CombatState, card: &CombatCard, purge: bool, target: Option<usize>) {
    if !purge && crate::content::cards::get_card_definition(card.id).card_type == crate::content::cards::CardType::Attack {
        ...
        // Deduct power
        if let Some(powers) = state.power_db.get_mut(&0) { // 0 is player EntityId 
