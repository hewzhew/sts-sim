...
pub fn at_end_of_turn(state: &CombatState, owner: crate::core::EntityId, amount: i32) -> SmallVec<[Action; 2]> {
    let mut actions = SmallVec::new();

    // Get hpLoss from extra_data (defaults to 1 if somehow missing)
    let hp_loss = state.power_db.get(&owner)
        .and_then(|ps| ps.iter().find(|p| p.power_type == PowerId::Combust))
        .map(|p| p.extra_data)
        .unwrap_or(1);

    actions.push(Action::LoseHp { target: owner, amount: hp_loss });
