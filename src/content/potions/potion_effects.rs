...
fn bottom(actions: &mut SmallVec<[ActionInfo; 4]>, action: Action) {
    actions.push(ActionInfo { action, insertion_mode: AddTo::Bottom });
}
...
#[allow(unused)]
pub fn get_potion_actions(potion: PotionId, target_idx: Option<usize>, potency: i32) -> SmallVec<[ActionInfo; 4]> {
    ...
    match potion {
        ...
        PotionId::FirePotion => {
            ...
            bottom(&mut actions, Action::Damage(DamageInfo {
                source: PLAYER, target, base: potency, output: potency,
                damage_type: DamageType::Thorns, is_modified: false,
            }));
        },
        PotionId::ExplosivePotion => {
            // Deal 10 damage to ALL enemies (thrown)
            bottom(&mut actions, Action::DamageAllEnemies {
                source: PLAYER,
        ...
        PotionId::PoisonPotion => {
            ...
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target, power_id: PowerId::Poison, amount: potency,
            });
        },
        PotionId::WeakenPotion => {
            // Apply 3 Weak to target enemy (thrown)
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target, power_id: PowerId::Weak, amount: potency,
            });
        },
        PotionId::FearPotion => {
            // Apply 3 Vulnerable to target enemy (thrown)
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target, power_id: PowerId::Vulnerable, amount: potency,
            });
        },
        PotionId::BlockPotion => {
            // Gain 12 Block
            bottom(&mut actions, Action::GainBlock { target: PLAYER, amount: potency });
        },
        PotionId::BloodPotion => {
            // Heal for potency% of Max HP. Negative sentinel triggers maxHP% calculation in handler.
            bottom(&mut actions, Action::Heal { target: PLAYER, amount: -(potency) });
        },
        ...
        PotionId::StrengthPotion => {
            ...
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::Strength, amount: potency,
            });
        },
        PotionId::DexterityPotion => {
            // Gain 2 Dexterity (permanent)
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::Dexterity, amount: potency,
            });
        },
        PotionId::SpeedPotion => {
            ...
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::DexterityDown, amount: potency,
            });
        },
        PotionId::SteroidPotion => {
            ...
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::LoseStrength, amount: potency,
            });
        },
        ...
        PotionId::FocusPotion => {
            // Gain 2 Focus (Defect)
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::Focus, amount: potency,
            });
        },
        PotionId::AttackPotion => {
            // Java: DiscoveryAction(CardType.ATTACK, potency) — opens choice screen with 3 Attack cards.
            bottom(&mut actions, Action::SuspendForDiscovery {
        ...
        PotionId::BottledMiracle => {
            ...
            bottom(&mut actions, Action::MakeTempCardInHand { card_id: crate::content::cards::CardId::Miracle, amount: potency as u8, upgraded: false });
        },
        ...
        PotionId::AncientPotion => {
            // Gain 1 Artifact
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::Artifact, amount: potency,
            });
        },
        PotionId::RegenPotion => {
            // Gain 5 Regen
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::Regen, amount: potency,
            });
        },
        PotionId::EssenceOfSteel => {
            // Gain 4 Plated Armor
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::PlatedArmor, amount: potency,
            });
        },
        PotionId::LiquidBronze => {
            // Gain 3 Thorns
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::Thorns, amount: potency,
            });
        },
        ...
        PotionId::DuplicationPotion => {
            ...
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::DuplicationPower, amount: potency,
            });
        },
        PotionId::CunningPotion => {
            // Java: creates UPGRADED Shiv, then MakeTempCardInHandAction(shiv, potency)
            bottom(&mut actions, Action::MakeTempCardInHand {
        ...
        PotionId::LiquidMemories => {
            ...
            bottom(&mut actions, Action::SuspendForGridSelect {
                source_pile: crate::state::PileType::Discard,
                min: 1, max: potency as u8, can_cancel: false,
        ...
        PotionId::GamblersBrew => {
            ...
            bottom(&mut actions, Action::SuspendForHandSelect {
                min: 0, max: 99, reason: crate::state::HandSelectReason::GamblingChip,
            });
        },
        PotionId::Elixir => {
            ...
            bottom(&mut actions, Action::SuspendForHandSelect {
                min: 0, max: 99, reason: crate::state::HandSelectReason::Exhaust,
            });
        },
        ...
        PotionId::GhostInAJar => {
            // Gain 1 Intangible
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::Intangible, amount: potency,
            });
        },
        PotionId::HeartOfIron => {
            // Gain 6 Metallicize
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::Metallicize, amount: potency,
            });
        },
        PotionId::CultistPotion => {
            // Gain 1 Ritual
            bottom(&mut actions, Action::ApplyPower {
                source: PLAYER, target: PLAYER, power_id: PowerId::Ritual, amount: potency,
            });
        },
        PotionId::Ambrosia => {
