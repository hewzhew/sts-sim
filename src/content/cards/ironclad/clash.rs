...
pub fn clash_play(state: &CombatState, card: &CombatCard, target: Option<crate::core::EntityId>) -> SmallVec<[ActionInfo; 4]> {
    ...
    let has_non_attacks = state.hand.iter().any(|c| {
        crate::content::cards::get_card_definition(c.id).card_type != crate::content::cards::CardType::Attack
    });
    if has_non_attacks {
        return actions;
    }
