...
pub fn corruption_play(state: &CombatState, _card: &CombatCard) -> SmallVec<[ActionInfo; 4]> {
    ...
    if state.get_power(0, crate::content::powers::PowerId::Corruption) == 0 {
        actions.push(ActionInfo {
            action: Action::ApplyPower { source: 0, target: 0, power_id: crate::content::powers::PowerId::Corruption, amount: 1 },
...
pub fn corruption_on_apply(state: &mut CombatState) {
    let is_skill = |id| crate::content::cards::get_card_definition(id).card_type == crate::content::cards::CardType::Skill;
...
pub fn corruption_on_use_card(state: &CombatState, card: &CombatCard, exhaust_override: &mut bool) {
    ...
    let has_corruption = state.power_db.get(&0).map_or(false, |powers| {
        powers.iter().any(|p| p.power_type == crate::content::powers::PowerId::Corruption)
    });
    if !has_corruption { return; }
    let def = crate::content::cards::get_card_definition(card.id);
    if def.card_type == crate::content::cards::CardType::Skill {
