...
pub fn get_choices(run_state: &RunState, event_state: &EventState) -> Vec<EventChoiceMeta> {
    match event_state.current_screen {
        ...
        1 => {
            let gold_reward = if run_state.ascension_level >= 15 { 50 } else { 75 };
            let damage = (run_state.max_hp / 10).max(1);
            vec![
                EventChoiceMeta::new(format!("[Touch] Lose {} HP. Gain {} Gold.", damage, gold_reward)),
        ...
        1 => {
            match choice_idx {
                ...
                1 => {
                    ...
                    let face_relics = [
                        RelicId::CultistMask, RelicId::FaceOfCleric,
                        RelicId::GremlinMask, RelicId::NlothsMask, RelicId::SsserpentHead,
                    ];
                    let mut available: Vec<RelicId> = face_relics.iter().copied()
                    ...
                    let relic_id = if available.is_empty() {
                        ...
                        crate::rng::shuffle_with_random_long(&mut available, &mut run_state.rng_pool.misc_rng);
                        available[0]
                    };
