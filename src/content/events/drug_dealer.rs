...
pub fn get_choices(run_state: &RunState, event_state: &EventState) -> Vec<EventChoiceMeta> {
    ...
    let purgeable_count = run_state.master_deck.iter().filter(|c| {
    ...
    if purgeable_count >= 2 {
        choices.push(EventChoiceMeta::new("[Become a Test Subject] Transform 2 cards."));
    } else {
    ...
    choices.push(EventChoiceMeta::new("[Inject Mutagens] Obtain Mutagenic Strength relic."));
    choices
}
    ...
    match event_state.current_screen {
        0 => {
            match choice_idx {
                ...
                1 => { // Transform 2 cards (Java: gridSelectScreen.open(getPurgeableCards(), 2, ...))
                ...
                2 => { // Obtain MutagenicStrength relic
                    let relic_id = if run_state.relics.iter().any(|r| r.id == crate::content::relics::RelicId::MutagenicStrength) {
