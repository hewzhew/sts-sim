...
pub fn get_choices(run_state: &RunState, event_state: &EventState) -> Vec<EventChoiceMeta> {
    match event_state.current_screen {
        ...
        4 => {
            let final_dmg = if run_state.ascension_level >= 15 { 15 } else { 10 };
            vec![
                EventChoiceMeta::new(format!("[Take the Book] Take {} damage. Obtain a Book relic.", final_dmg)),
        ...
        4 => {
            match choice_idx {
                0 => {
                    ...
                    let book_relics = [
                        RelicId::Necronomicon, RelicId::Enchiridion, RelicId::NilrysCodex,
                    ];
                    let available: Vec<RelicId> = book_relics.iter().copied()
                        .filter(|r| !run_state.relics.iter().any(|owned| owned.id == *r))
                        .collect();
                    let relic_id = if available.is_empty() {
                        RelicId::Circlet
                    } else {
                        let idx = run_state.rng_pool.misc_rng.random_range(0, available.len() as i32 - 1) as usize;
                        available[idx]
                    };
