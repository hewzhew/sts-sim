...
pub fn get_choices(run_state: &RunState, event_state: &EventState) -> Vec<EventChoiceMeta> {
    match event_state.current_screen {
        0 => {
            ...
            let hp_loss_pct = if run_state.ascension_level >= 15 { 0.35 } else { 0.25 };
            let hp_loss = (run_state.max_hp as f32 * hp_loss_pct).round() as i32;
            let mut choices = vec![];
            if has_idol {
                choices.push(EventChoiceMeta::new("[Offer] Trade Golden Idol for Bloody Idol."));
            } else {
                choices.push(EventChoiceMeta::disabled("[Offer] Requires Golden Idol.", "No Golden Idol"));
            }
            choices.push(EventChoiceMeta::new(format!("[Pray] Gain 5 Max HP. Lose {} HP.", hp_loss)));
        ...
        0 => {
            match choice_idx {
                0 => {
                    ...
                    if let Some(pos) = run_state.relics.iter().position(|r| r.id == RelicId::GoldenIdol) {
