    ...
    match event_state.current_screen {
        0 => {
            ...
            if run_state.gold >= heal_cost {
                choices.push(EventChoiceMeta::new(format!("[Heal] Lose {} Gold. Heal 25% of your Max HP.", heal_cost)));
            } else {
                choices.push(EventChoiceMeta::disabled(format!("[Heal] Lose {} Gold. Heal 25% of your Max HP.", heal_cost), "Not enough Gold."));
            }

            let purify_cost = if run_state.ascension_level >= 15 { 75 } else { 50 };
            if run_state.gold >= purify_cost {
                choices.push(EventChoiceMeta::new(format!("[Purify] Lose {} Gold. Remove a card from your deck.", purify_cost)));
            } else {
                choices.push(EventChoiceMeta::disabled(format!("[Purify] Lose {} Gold. Remove a card from your deck.", purify_cost), "Not enough Gold."));
            }
