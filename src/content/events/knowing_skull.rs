...
fn potion_n(state: i32) -> i32 { state & 0xFF }
fn gold_n(state: i32) -> i32 { (state >> 8) & 0xFF }
fn card_n(state: i32) -> i32 { (state >> 16) & 0xFF }

fn inc_potion(state: &mut i32) { *state += 1; }
fn inc_gold(state: &mut i32) { *state += 1 << 8; }
fn inc_card(state: &mut i32) { *state += 1 << 16; }

fn potion_cost(state: i32) -> i32 { BASE_COST + potion_n(state) }
fn gold_cost(state: i32) -> i32 { BASE_COST + gold_n(state) }
fn card_cost(state: i32) -> i32 { BASE_COST + card_n(state) }
...
pub fn get_choices(_run_state: &RunState, event_state: &EventState) -> Vec<EventChoiceMeta> {
    match event_state.current_screen {
        ...
        1 => {
            ...
            vec![
                EventChoiceMeta::new(format!("[Potion] Lose {} HP. Obtain a random Potion.", potion_cost(s))),
                EventChoiceMeta::new(format!("[Gold] Gain {} Gold. Lose {} HP.", GOLD_REWARD, gold_cost(s))),
                EventChoiceMeta::new(format!("[Card] Lose {} HP. Obtain a colorless card.", card_cost(s))),
        ...
        1 => {
            match choice_idx {
                ...
                2 => {
                    ...
                    let card_id = run_state.random_colorless_card(crate::content::cards::CardRarity::Uncommon);
