...
fn num_rewards(s: i32) -> i32 { s & 0xF }
fn encounter_chance(s: i32) -> i32 { (s >> 4) & 0xFF }
fn reward_type(s: i32, idx: i32) -> i32 { (s >> (12 + idx * 2)) & 0x3 }

fn set_num_rewards(s: &mut i32, n: i32) { *s = (*s & !0xF) | (n & 0xF); }
fn set_encounter_chance(s: &mut i32, c: i32) { *s = (*s & !(0xFF << 4)) | ((c & 0xFF) << 4); }
            ...
            match choice_idx {
                0 => {
                    ...
                    if roll < chance {
                        ...
                        rewards.items.push(crate::state::reward::RewardItem::Gold { amount: combat_gold });
                        ...
                        for i in n..3 {
                            ...
                            match rt {
                                0 => rewards.items.push(crate::state::reward::RewardItem::Gold { amount: GOLD_REWARD }),
                                2 => {
...
fn apply_reward(run_state: &mut RunState, reward_type: i32) {
    match reward_type {
        0 => { run_state.gold += GOLD_REWARD; }, // Gold
        2 => {
            // Relic
            let relic_id = run_state.random_relic();
...
pub fn init_dead_adventurer_state(run_state: &mut RunState) -> i32 {
    let base_chance = if run_state.ascension_level >= 15 { 35 } else { 25 };
