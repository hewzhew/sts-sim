...
pub fn on_monster_death(state: &crate::combat::CombatState, dead_monster_id: crate::core::EntityId) -> SmallVec<[ActionInfo; 4]> {
    let mut actions = SmallVec::new();

    // Find dead monster's Poison amount from power_db
    let poison_amount = state.power_db.get(&dead_monster_id)
        .and_then(|powers| powers.iter().find(|p| p.power_type == crate::content::powers::PowerId::Poison))
        .map(|p| p.amount)
        .unwrap_or(0);

    if poison_amount > 0 {
        // Find a random alive enemy to transfer Poison to
        let alive: Vec<_> = state.monsters.iter()
