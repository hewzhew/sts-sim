...
pub fn at_end_of_turn(_state: &CombatState, player: &PlayerEntity) -> SmallVec<[ActionInfo; 4]> {
    ...
    if !player.orbs.is_empty() && player.orbs[0].id != crate::combat::OrbId::Empty {
        let orb = &player.orbs[0];
        match orb.id {
            crate::combat::OrbId::Lightning => {
                actions.push(ActionInfo {
                    action: Action::AttackDamageRandomEnemy { base_damage: orb.passive_amount, damage_type: crate::action::DamageType::Thorns },
            ...
            crate::combat::OrbId::Frost => {
                actions.push(ActionInfo {
                    action: Action::GainBlock { target: player.id, amount: orb.passive_amount },
                    insertion_mode: AddTo::Bottom,
                });
            },
            crate::combat::OrbId::Plasma => {
                actions.push(ActionInfo {
                    action: Action::GainEnergy { amount: orb.passive_amount },
                    insertion_mode: AddTo::Bottom,
                });
            },
            crate::combat::OrbId::Empty => {},
        }
    }
    actions
}
