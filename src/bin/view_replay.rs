...
fn main() {
    ...
    match combat {
        Some(c) => {
            println!("--- Combat #{} Floor {} vs {:?} ---", c.combat_idx, c.floor, c.monster_names);
            println!("Initial Start:");
            let m_init = &c.start_snapshot["combat_state"]["monsters"][0];
            println!("   Monster 0 (HP: {}, Block: {}, Intent: {})", m_init["current_hp"], m_init["block"], m_init["intent"]);
            ...
            for (i, action) in c.actions.iter().enumerate() {
                ...
                let detail = match action.action_type.as_str() {
                    "play" => format!("PLAY Card {} -> Target {:?}", action.card_index.unwrap_or(999), action.target),
                ...
                println!("[{:02}] {:<30} | => Java Result | Monster HP: {:3}, Block: {:2}, Intent: {}", 
                    i + 1, detail, hp, block, intent);
                
                // Show power changes optionally
                if let Some(powers) = m["powers"].as_array() {
                    let p_str: Vec<String> = powers.iter()
