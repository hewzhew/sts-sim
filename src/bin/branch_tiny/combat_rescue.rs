...
pub(super) struct NonBossCombatRescue {
...
pub(super) fn try_nonboss_combat_rescue(
    session: &mut RunControlSession,
    args: Args,
) -> Result<NonBossCombatRescue, String> {
    let mut auto_steps = Vec::new();
    let mut combat_search = Vec::new();
    let rescue = apply_owner_audit_auto_run(session, diagnostic_rescue_auto_step_options(args))
        .map_err(|err| format!("diagnostic combat rescue failed: {err}"))?;
    combat_search.extend(combat_search_summaries(&rescue));
    auto_steps.extend(rescue.auto_applied_steps.clone());
    let mut status = boundary_router::classify_auto_outcome(session, &rescue);

    if matches!(status, BranchStatus::CombatGap { .. })
        && should_try_hallway_immediate_rescue(session)
    {
        let rescue =
            apply_owner_audit_auto_run(session, hallway_immediate_rescue_auto_step_options(args))
                .map_err(|err| format!("hallway immediate rescue failed: {err}"))?;
        combat_search.extend(combat_search_summaries(&rescue));
        auto_steps.extend(rescue.auto_applied_steps.clone());
