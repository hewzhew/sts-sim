...
fn campaign_route_decision_journal_event_v1(
    ...
    CampaignJournalEventV1 {
        ...
        payload: CampaignJournalEventPayloadV1::RouteDecision {
            ...
            candidate_pool_provenance: decision.candidate_pool_provenance.clone(),
...
mod tests {
    ...
    #[test]
    fn route_candidate_pool_journal_preserves_typed_route_candidates_without_map_packet() {
        ...
        match event.payload {
            CampaignJournalEventPayloadV1::RouteCandidatePool {
                ...
                assert!(
                    map_decision_packet.is_none(),
                    "journal should avoid embedding the full route trace packet in every route pool"
                );
                assert_eq!(packet.candidates.len(), route_candidates.len());
