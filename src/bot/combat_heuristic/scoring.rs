...
#[cfg(test)]
mod tests {
    use super::play_priority;
    use crate::bot::combat_heuristic::sim::build_sim_state;
    use crate::combat::{CombatCard, Intent};
    use crate::content::cards::CardId;
    use crate::content::monsters::EnemyId;
    use crate::testing::support::test_support::{
        combat_with_hand, combat_with_hand_and_intent, CombatTestExt,
    };

    #[test]
    fn rage_gets_priority_when_multiple_followup_attacks_exist() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::Rage, 1),
        ...
        assert!(play_priority(&state, 0) > play_priority(&state, 1));
    }

    #[test]
    fn spot_weakness_priority_drops_when_enemy_is_not_attacking() {
        let attacking = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::SpotWeakness, 1),
                CombatCard::new(CardId::HeavyBlade, 2),
            ],
            Intent::Attack {
                damage: 10,
                hits: 1,
        ...
        let passive = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::SpotWeakness, 1),
                CombatCard::new(CardId::HeavyBlade, 2),
            ],
        ...
        let attacking_state = build_sim_state(&attacking);
        let passive_state = build_sim_state(&passive);

        assert!(play_priority(&attacking_state, 0) > play_priority(&passive_state, 0));
    }

    #[test]
    fn second_wind_priority_drops_for_clean_high_hp_hand_without_engine() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::SecondWind, 1),
                CombatCard::new(CardId::Defend, 2),
                CombatCard::new(CardId::Defend, 3),
                CombatCard::new(CardId::Armaments, 4),
                CombatCard::new(CardId::InfernalBlade, 5),
            ],
            Intent::Attack {
                damage: 10,
                hits: 1,
    ...
    #[test]
    fn power_through_priority_drops_when_no_incoming_damage_exists() {
        let combat = combat_with_hand_and_intent(
            vec![
    ...
    #[test]
    fn fire_breathing_priority_rises_in_status_boss_fight() {
        let combat =
            combat_with_hand(&[CardId::FireBreathing, CardId::DarkEmbrace, CardId::Defend])
                .with_boss_fight(true)
                .with_monster_type(1, EnemyId::SlimeBoss)
                .with_monster_max_hp(1, 140)
                .with_monster_hp(1, 140)
                .with_monster_intent(
                    1,
                    Intent::Attack {
                        damage: 35,
                ...
                .with_draw_ids(&[CardId::Slimed, CardId::Slimed, CardId::WildStrike]);
        let state = build_sim_state(&combat);
    ...
    #[test]
    fn brutality_priority_drops_at_low_hp() {
        let high_hp = combat_with_hand(&[CardId::Brutality]).with_player_hp(40);
        let low_hp = combat_with_hand(&[CardId::Brutality]).with_player_hp(7);

        let high_state = build_sim_state(&high_hp);
        let low_state = build_sim_state(&low_hp);

        assert!(play_priority(&high_state, 0) > play_priority(&low_state, 0));
    }

    #[test]
    fn panacea_priority_rises_with_self_debuff_combo() {
        let combo = combat_with_hand(&[CardId::Panacea, CardId::Flex, CardId::Strike]);
        let plain = combat_with_hand(&[CardId::Panacea, CardId::Strike, CardId::Defend]);

        let combo_state = build_sim_state(&combo);
        let plain_state = build_sim_state(&plain);

        assert!(play_priority(&combo_state, 0) > play_priority(&plain_state, 0));
    }

    #[test]
    fn bloodletting_priority_drops_without_same_turn_cashout() {
        let combat = combat_with_hand_and_intent(
    ...
    #[test]
    fn bloodletting_priority_rises_when_it_unlocks_big_followup() {
        let high_cashout = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::Bloodletting, 1),
                CombatCard::new(CardId::Carnage, 2),
            ],
            Intent::Buff,
        )
        .with_energy(1);
        let low_cashout = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::Bloodletting, 1),
                CombatCard::new(CardId::Defend, 2),
            ],
        ...
        let high_state = build_sim_state(&high_cashout);
        let low_state = build_sim_state(&low_cashout);

        assert!(play_priority(&high_state, 0) > play_priority(&low_state, 0));
    }

    #[test]
    fn inflame_priority_rises_over_basic_attacks_when_followups_exist() {
        let combat = combat_with_hand_and_intent(
            vec![
    ...
    #[test]
    fn pommel_strike_priority_rises_when_it_can_open_better_followup() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::PommelStrike, 1),
                CombatCard::new(CardId::Strike, 2),
            ],
            Intent::Buff,
        )
        .with_energy(2)
        .with_draw_ids(&[CardId::Bludgeon, CardId::Defend]);
        let state = build_sim_state(&combat);
    ...
    #[test]
    fn dark_embrace_priority_rises_with_true_grit_and_burn_in_hand() {
        ...
        assert!(play_priority(&state, 0) > play_priority(&state, 3));
    }

    #[test]
    fn feel_no_pain_priority_drops_without_same_turn_exhaust_payoff() {
        let combat = combat_with_hand_and_intent(
            vec![
        ...
        assert!(play_priority(&state, 0) < play_priority(&state, 2));
    }

    #[test]
    fn disarm_priority_drops_on_low_pressure_single_hit_turn() {
        let combat = combat_with_hand_and_intent(
            vec![
    ...
    #[test]
    fn feed_priority_rises_on_safe_kill() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::Feed, 1),
                CombatCard::new(CardId::Strike, 2),
            ],
            Intent::Buff,
        )
        .with_monster_hp(1, 8)
        .with_monster_max_hp(1, 8)
        .with_energy(2);
        let state = build_sim_state(&combat);
