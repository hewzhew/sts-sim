...
mod tests {
    use super::{
        choose_event_option, choose_live_event_choice, classify_event_option_tags,
        derive_event_features, local_event_context, parse_event_option_payload,
        EventDecisionContext, EventOptionTag, EventOptionView, EventPolicyFamily,
    };
    ...
    fn make_option(index: usize, text: &str) -> EventOptionView {
        let label = super::extract_bracket_label(text);
        EventOptionView {
            index,
            text: text.to_string(),
            label: label.clone(),
    ...
    fn make_context(
        ...
        EventDecisionContext {
            ...
            current_screen,
            current_screen_index: Some(current_screen),
            current_screen_key: None,
            screen_source: Some("test".to_string()),
            options: texts
                .iter()
                .enumerate()
                .map(|(idx, text)| make_option(idx, text))
                .collect(),
            features: derive_event_features(rs),
        }
    }
    ...
    #[test]
    fn golden_idol_screen_zero_prefers_take_without_ectoplasm() {
        let rs = run_state();
        let event = EventState::new(EventId::GoldenIdol);
        let context = local_event_context(
            &rs,
            &event,
            &[
        ...
        let decision = choose_event_option(&rs, &context).expect("decision");
    ...
    #[test]
    fn golden_idol_screen_zero_prefers_leave_with_ectoplasm() {
        let mut rs = run_state();
        rs.obtain_relic(RelicId::Ectoplasm, EngineState::MapNavigation);
        let event = EventState::new(EventId::GoldenIdol);
        let context = local_event_context(
            &rs,
            &event,
        ...
        assert_eq!(
            ...
            Some("cost_tradeoff_avoid_ectoplasm_gold_relic")
        );
    }

    #[test]
    fn golden_idol_trap_screen_never_returns_first_screen_index() {
        let mut rs = run_state();
        rs.current_hp = 73;
        rs.max_hp = 80;
        let mut event = EventState::new(EventId::GoldenIdol);
        event.current_screen = 1;
        let context = local_event_context(
            &rs,
            &event,
            &[
                EventChoiceMeta::new("[Run] Become Cursed - Injury."),
                EventChoiceMeta::new("[Fight] Take 12 damage."),
                EventChoiceMeta::new("[Sacrifice] Lose 8 Max HP."),
            ],
        );
        let decision = choose_event_option(&rs, &context).expect("decision");
        assert!(decision.option_index <= 2);
        assert_ne!(decision.option_index, 0);
    }

    #[test]
    fn local_and_live_golden_idol_contexts_agree() {
        let rs = run_state();
        let event = EventState::new(EventId::GoldenIdol);
        let local = choose_event_option(
            &rs,
            &local_event_context(
                &rs,
                &event,
        ...
        let live = choose_live_event_choice(&gs, &rs).expect("live decision");
        assert_eq!(local.option_index, live.option_index);
        assert_eq!(local.command_index, live.command_index);
    }

    #[test]
    fn fallback_avoids_disabled_option() {
        let rs = run_state();
        let mut context = make_context(&rs, "Unknown", 0, &["[Leave]", "[Take] Gain 50 Gold."]);
        context.options[0].disabled = true;
        let decision = choose_event_option(&rs, &context).expect("decision");
        assert_eq!(decision.option_index, 1);
    }

    fn attach_simple_route(
        rs: &mut RunState,
        immediate_room: RoomType,
        second_room: Option<RoomType>,
        elite_room: Option<RoomType>,
        rest_room: Option<RoomType>,
        shop_room: Option<RoomType>,
    ) {
        let mut row0 = vec![MapRoomNode::new(0, 0)];
        row0[0].class = Some(immediate_room);
        let mut row1 = vec![
            MapRoomNode::new(0, 1),
            MapRoomNode::new(1, 1),
            MapRoomNode::new(2, 1),
        ...
        row1[0].class = second_room;
        row1[1].class = elite_room;
        row1[2].class = rest_room;
        row1[3].class = shop_room;
        row0[0].edges.insert(MapEdge::new(0, 0, 0, 1));
        row0[0].edges.insert(MapEdge::new(0, 0, 1, 1));
        row0[0].edges.insert(MapEdge::new(0, 0, 2, 1));
        row0[0].edges.insert(MapEdge::new(0, 0, 3, 1));
        rs.map = MapState::new(vec![row0, row1]);
        rs.map.current_x = 0;
        rs.map.current_y = 0;
    }

    #[test]
    fn scrap_ooze_prefers_continue_when_healthy() {
        let mut rs = run_state();
        rs.current_hp = 80;
        rs.max_hp = 80;
        attach_simple_route(
        ...
        let context = make_context(
            ...
            0,
            &[
                "[Reach In] Take 3 damage. 25% chance to obtain a Relic.",
                "[Leave]",
            ],
        );
        let decision = choose_event_option(&rs, &context).expect("decision");
        ...
        assert_eq!(
            decision.rationale_key,
            Some("press_your_luck_continue_positive_ev")
        );
    }

    #[test]
    fn scrap_ooze_prefers_leave_when_damaged_and_route_is_hostile() {
        let mut rs = run_state();
        rs.current_hp = 24;
        rs.max_hp = 80;
        attach_simple_route(
        ...
        let context = make_context(
            ...
            0,
            &[
                "[Reach In] Take 6 damage. 55% chance to obtain a Relic.",
                "[Leave]",
            ],
        );
        let decision = choose_event_option(&rs, &context).expect("decision");
        assert_eq!(decision.option_index, 1);
        assert_eq!(decision.rationale_key, Some("press_your_luck_stop"));
    }

    #[test]
    fn local_and_live_scrap_ooze_contexts_agree() {
        let mut rs = run_state();
        rs.current_hp = 70;
        rs.max_hp = 80;
        attach_simple_route(
        ...
        let local = choose_event_option(
            ...
            &make_context(
                ...
                0,
                &[
                    "[Reach In] Take 4 damage. 35% chance to obtain a Relic.",
                    "[Leave]",
                ],
            ),
        )
        ...
        let gs = json!({
            "screen_state": {
                ...
                "current_screen": 0,
                "options": [
                    {"text": "[Reach In] Take 4 damage. 35% chance to obtain a Relic.", "label": "Reach In", "disabled": false, "choice_index": 0},
                    {"text": "[Leave]", "label": "Leave", "disabled": false, "choice_index": 1}
                ]
            }
        });
    ...
    #[test]
    fn world_of_goop_prefers_gold_when_hp_is_healthy() {
        let mut rs = run_state();
        rs.current_hp = 67;
        rs.max_hp = 80;
        attach_simple_route(
    ...
    #[test]
    fn golden_shrine_prefers_desecrate_when_curse_is_tractable() {
        let mut rs = run_state();
        rs.current_hp = 72;
        rs.max_hp = 80;
        rs.gold = 140;
        ...
        assert_eq!(
            decision.rationale_key,
            Some("cost_tradeoff_accept_curse_for_gold")
        );
    }

    #[test]
    fn face_trader_prefers_trade_when_healthy() {
        let mut rs = run_state();
        rs.current_hp = 70;
        rs.max_hp = 80;
        let context = make_context(
            ...
            1,
            &[
                "[Touch] Lose 8 HP. Gain 75 Gold.",
                "[Trade] Obtain a face Relic.",
                "[Leave]",
            ],
        );
        let decision = choose_event_option(&rs, &context).expect("decision");
        ...
        assert_eq!(
            decision.rationale_key,
            Some("cost_tradeoff_trade_for_relic")
        );
    }

    #[test]
    fn we_meet_again_prefers_give_card_when_junk_exists() {
        let mut rs = run_state();
        rs.gold = 90;
        let context = make_context(
            &rs,
            ...
            0,
            &[
                "[Give Gold] Lose 55 Gold. Obtain a Relic.",
                "[Give Card] Remove a card. Obtain a Relic.",
                "[Attack]",
            ],
        );
        let decision = choose_event_option(&rs, &context).expect("decision");
        ...
        assert_eq!(
            decision.rationale_key,
            Some("resource_shoplike_trade_card_for_relic")
        );
    }

    #[test]
    fn drug_dealer_prefers_transform_for_basic_deck() {
        let rs = run_state();
        let context = make_context(
            &rs,
            "Drug Dealer",
    ...
    #[test]
    fn local_and_live_golden_shrine_contexts_agree() {
        let rs = run_state();
        let local = choose_event_option(
            &rs,
            &make_context(
        ...
        let gs = json!({
            "screen_state": {
                ...
                "current_screen": 0,
                "options": [
                    {"text": "[Pray] Gain 100 Gold.", "label": "Pray", "disabled": false, "choice_index": 0},
                    {"text": "[Desecrate] Gain 275 Gold. Become Cursed - Regret.", "label": "Desecrate", "disabled": false, "choice_index": 1},
                    {"text": "[Leave]", "label": "Leave", "disabled": false, "choice_index": 2}
                ]
            }
        });
        let live = choose_live_event_choice(&gs, &rs).expect("live decision");
    ...
    #[test]
    fn dead_adventurer_stops_when_fight_risk_is_too_high() {
        let mut rs = run_state();
        rs.current_hp = 18;
        rs.max_hp = 80;
        attach_simple_route(
        ...
        let context = make_context(
            ...
            "Dead Adventurer",
            0,
            &["[Search] 75% chance of a fight.", "[Leave]"],
        );
        let decision = choose_event_option(&rs, &context).expect("decision");
        assert_eq!(decision.option_index, 1);
    }
    ...
    #[test]
    fn knowing_skull_avoids_potion_loop_when_safe() {
        let mut rs = run_state();
        rs.current_hp = 55;
        rs.max_hp = 80;
        let context = make_context(
            ...
            1,
            &[
                "[Potion] Lose 12 HP. Obtain a random Potion.",
                "[Gold] Gain 90 Gold. Lose 9 HP.",
                "[Card] Lose 6 HP. Obtain a colorless card.",
                "[Leave] Lose 6 HP.",
            ],
        );
        let decision = choose_event_option(&rs, &context).expect("decision");
        ...
        assert!(matches!(
            ...
            Some("press_your_luck_skull_take_card")
                | Some("press_your_luck_skull_take_gold")
                | Some("press_your_luck_stop")
        ));
    }

    #[test]
    fn knowing_skull_leaves_when_only_leave_is_survivable() {
        let mut rs = run_state();
        rs.current_hp = 7;
        rs.max_hp = 80;
        let context = make_context(
            ...
            &[
                "[Potion] Lose 12 HP. Obtain a random Potion.",
                "[Gold] Gain 90 Gold. Lose 15 HP.",
                "[Card] Lose 8 HP. Obtain a colorless card.",
                "[Leave] Lose 6 HP.",
            ],
        );
        let decision = choose_event_option(&rs, &context).expect("decision");
        assert_eq!(decision.option_index, 3);
        assert_eq!(decision.rationale_key, Some("press_your_luck_stop"));
        assert!(!decision.safety_override_applied || decision.option_index == 3);
    }

    #[test]
    fn option_payload_parses_hp_and_gold() {
        let payload = parse_event_option_payload("[Gold] Gain 90 Gold. Lose 13 HP.", Some("Gold"));
        assert_eq!(payload.gold_delta, 90);
        assert_eq!(payload.hp_cost, 13);
    }

    #[test]
    fn option_tags_cover_colorless_card_and_starter_relic_loss() {
        let tags = classify_event_option_tags(
            "[Take] Lose your starter relic. Obtain a random Boss Relic.",
            Some("Take"),
        );
        assert!(tags.contains(&EventOptionTag::TakeRelic));
        assert!(tags.contains(&EventOptionTag::LoseStarterRelic));
    }

    #[test]
    fn local_and_live_normalization_match_for_knowing_skull() {
        let mut rs = run_state();
        rs.current_hp = 30;
        let local = make_context(
            &rs,
            "Knowing Skull",
            1,
            &[
        ...
        let gs = json!({
            "screen_state": {
                ...
                "current_screen": 1,
                "options": [
                    {"text": "[Potion] Lose 12 HP. Obtain a random Potion.", "label": "Potion", "disabled": false, "choice_index": 0},
                    {"text": "[Leave] Lose 6 HP.", "label": "Leave", "disabled": false, "choice_index": 1}
                ]
            }
        });
        let live = super::live_event_context(&gs, &rs).expect("live context");
        assert_eq!(local.options[0].payload, live.options[0].payload);
        assert_eq!(local.options[1].payload, live.options[1].payload);
    }
}
