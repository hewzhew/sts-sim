...
use super::coverage_signatures::InteractionSignature;

pub fn archetype_tags_for_combat(combat: &crate::runtime::combat::CombatState) -> Vec<String> {
    crate::bot::archetype_tags(&crate::bot::combat_zone_profile(combat))
}
...
impl CoverageDb {
    ...
    pub fn record_signature(&mut self, signature: &InteractionSignature) {
    ...
    pub fn record_source_combo(&mut self, signature: &InteractionSignature) {
        let source_key = signature.source_combo_key();
        *self.source_signature_counts.entry(source_key).or_insert(0) += 1;
    }
}
