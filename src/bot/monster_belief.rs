...
mod tests {
    ...
    use crate::combat::{
        ...
        EngineRuntime, EntityState, EphemeralCounters, LagavulinRuntimeState,
        MonsterProtocolIdentity, PlayerEntity, RelicBuses, TurnRuntime,
    };
    use crate::content::monsters::EnemyId;
    ...
    fn test_combat(monsters: Vec<MonsterEntity>) -> CombatState {
        CombatState {
            meta: CombatMeta {
                ascension_level: 0,
            ...
            rng: CombatRng::new(RngPool::new(1)),
            runtime: CombatRuntimeHints::default(),
        }
    }

    fn monster(enemy_id: EnemyId) -> MonsterEntity {
        MonsterEntity {
            id: 1,
    ...
    #[test]
    fn cultist_is_exact_from_history() {
        let combat = test_combat(vec![monster(EnemyId::Cultist)]);
        let obs = build_monster_observation(&combat, &combat.entities.monsters[0]);
        let belief = predict_monster_belief(&obs, 0);
        assert_eq!(belief.certainty, MonsterBeliefCertainty::Exact);
        assert_eq!(belief.predicted_moves[0].move_id, 3);
    }

    #[test]
    fn sentry_is_exact_from_slot_and_history() {
        let mut sentry = monster(EnemyId::Sentry);
        sentry.slot = 1;
        let combat = test_combat(vec![sentry]);
        let obs = build_monster_observation(&combat, &combat.entities.monsters[0]);
        let belief = predict_monster_belief(&obs, 0);
        assert_eq!(belief.certainty, MonsterBeliefCertainty::Exact);
        assert_eq!(belief.predicted_moves[0].move_id, 4);
    }

    #[test]
    fn jaw_worm_distribution_has_attack_pressure() {
        let mut jaw = monster(EnemyId::JawWorm);
        jaw.move_history.push_back(2);
        let combat = test_combat(vec![jaw]);
        let obs = build_monster_observation(&combat, &combat.entities.monsters[0]);
        let belief = predict_monster_belief(&obs, 0);
        assert_eq!(belief.certainty, MonsterBeliefCertainty::Distribution);
        assert!(belief.attack_probability > 0.4);
        assert!(belief.expected_incoming_damage > 0.0);
    }

    #[test]
    fn lagavulin_sleep_is_exact_from_public_runtime() {
        let mut laga = monster(EnemyId::Lagavulin);
        laga.lagavulin.idle_count = 1;
        laga.lagavulin.is_out_triggered = false;
        let combat = test_combat(vec![laga]);
        let obs = build_monster_observation(&combat, &combat.entities.monsters[0]);
        let belief = predict_monster_belief(&obs, 0);
        assert_eq!(belief.certainty, MonsterBeliefCertainty::Exact);
        assert_eq!(belief.predicted_moves[0].move_id, 5);
    }

    #[test]
    fn visible_intent_belief_uses_exact_visible_state() {
        let mut cultist = monster(EnemyId::Cultist);
        cultist.next_move_byte = 1;
        cultist.current_intent = Intent::Attack { damage: 6, hits: 1 };
        let combat = test_combat(vec![cultist]);
        let belief = build_combat_belief_state(&combat);
        assert!(!belief.hidden_intent_active);
        assert_eq!(belief.expected_incoming_damage, 6.0);
        assert_eq!(
            belief.monsters[0].inference_source,
            MonsterInferenceSource::VisibleIntent
        );
    }

    #[test]
    fn runic_dome_forces_public_inference_over_visible_engine_truth() {
        let mut cultist = monster(EnemyId::Cultist);
        cultist.next_move_byte = 1;
        cultist.current_intent = Intent::Attack { damage: 6, hits: 1 };
        let mut combat = test_combat(vec![cultist]);
        combat
            .entities
            .player
            .relics
        ...
        assert_eq!(belief.monsters[0].predicted_moves[0].move_id, 3);
        assert_eq!(belief.monsters[0].expected_incoming_damage, 0.0);
    }
}
