...
mod tests {
    ...
    #[test]
    fn act1_pick_delta_prefers_frontload_patch() {
        let mut rs = run_with(&[CardId::Strike, CardId::Strike, CardId::Defend, CardId::Bash]);
        rs.act_num = 1;
        rs.floor_num = 6;

        assert!(
            compare_pick_vs_skip(&rs, CardId::Hemokinesis).total
                > compare_pick_vs_skip(&rs, CardId::Clash).total
        );
    }

    #[test]
    fn purge_delta_is_positive_in_basic_clogged_deck() {
        let rs = run_with(&[CardId::Strike, CardId::Strike, CardId::Defend, CardId::Bash]);
        assert!(compare_purge_vs_keep(&rs).total > 0);
    }

    #[test]
    fn transform_delta_improves_when_upgraded_context_exists() {
        let rs = run_with(&[CardId::Strike, CardId::Strike, CardId::Defend, CardId::Bash]);
        assert!(
            compare_transform_vs_decline(&rs, 1, true).total
                > compare_transform_vs_decline(&rs, 1, false).total
        );
    }

    #[test]
    fn vampires_delta_is_positive_for_dense_starter_strike_shell() {
        let rs = run_with(&[
            CardId::Strike,
            CardId::Strike,
            CardId::Strike,
        ...
        assert!(compare_vampires_vs_refuse(&rs).total > 0);
    }

    #[test]
    fn rollout_delta_participates_in_pick_score() {
        let mut rs = run_with(&[CardId::Strike, CardId::Strike, CardId::Defend, CardId::Bash]);
        rs.act_num = 1;
        rs.floor_num = 6;

        let score = compare_pick_vs_skip(&rs, CardId::ShrugItOff);
        assert_eq!(
            score.total,
            score.prior_delta + score.suite_bias + score.rollout_delta + score.context_delta
        );
    }

    #[test]
    fn rollout_delta_participates_in_upgrade_score() {
        let mut rs = run_with(&[CardId::Bash, CardId::Strike, CardId::Defend]);
        rs.act_num = 1;
        rs.floor_num = 8;

        let score = compare_upgrade_vs_decline(&rs, 1);
        assert_eq!(
            score.total,
            score.prior_delta + score.suite_bias + score.rollout_delta + score.context_delta
        );
    ...
    #[test]
    fn rollout_delta_participates_in_duplicate_score() {
        let mut rs = run_with(&[CardId::ShrugItOff, CardId::Bash, CardId::Defend]);
        rs.act_num = 2;
        rs.floor_num = 20;

        let score = compare_duplicate_vs_decline(&rs);
        assert_eq!(
            score.total,
            score.prior_delta + score.suite_bias + score.rollout_delta + score.context_delta
        );
    ...
    #[test]
    fn rollout_delta_participates_in_transform_score() {
        let mut rs = run_with(&[CardId::Strike, CardId::Strike, CardId::Defend, CardId::Bash]);
        rs.act_num = 1;
        rs.floor_num = 10;

        let score = compare_transform_vs_decline(&rs, 1, false);
        assert_eq!(
            score.total,
            score.prior_delta + score.suite_bias + score.rollout_delta + score.context_delta
        );
