...
mod tests {
    ...
    #[test]
    fn limit_break_is_bad_without_strength_and_good_with_it() {
        let empty_strength = run_with(&[CardId::Strike, CardId::Defend, CardId::Bash]);
        let strength_deck = run_with(&[
            CardId::Inflame,
            CardId::TwinStrike,
            CardId::HeavyBlade,
            CardId::Bash,
        ...
        let no_support = CardEvaluator::evaluate_card(CardId::LimitBreak, &empty_strength);
        let with_support = CardEvaluator::evaluate_card(CardId::LimitBreak, &strength_deck);

        assert!(with_support > no_support);
    }

    #[test]
    fn body_slam_prefers_block_shells() {
        let thin_deck = run_with(&[CardId::Strike, CardId::Strike, CardId::Bash]);
        let block_deck = run_with(&[
            CardId::ShrugItOff,
            CardId::FlameBarrier,
            CardId::Impervious,
            CardId::Barricade,
        ...
        let low = CardEvaluator::evaluate_card(CardId::BodySlam, &thin_deck);
        let high = CardEvaluator::evaluate_card(CardId::BodySlam, &block_deck);

        assert!(high > low);
    }

    #[test]
    fn feel_no_pain_prefers_exhaust_shells() {
        let thin_deck = run_with(&[CardId::Strike, CardId::Defend, CardId::Bash]);
        let exhaust_deck = run_with(&[
            CardId::SecondWind,
            CardId::TrueGrit,
            CardId::BurningPact,
            CardId::Corruption,
        ...
        let low = CardEvaluator::evaluate_card(CardId::FeelNoPain, &thin_deck);
        let high = CardEvaluator::evaluate_card(CardId::FeelNoPain, &exhaust_deck);

        assert!(high > low);
    }

    #[test]
    fn archetype_tags_detect_hybrid_shells() {
        let rs = run_with(&[
            CardId::Inflame,
            CardId::HeavyBlade,
            CardId::TwinStrike,
        ...
        let tags = CardEvaluator::archetype_tags(&CardEvaluator::deck_profile(&rs));
        assert!(tags.contains(&"strength".to_string()));
        assert!(tags.contains(&"exhaust".to_string()));
        assert!(tags.contains(&"block".to_string()));
        assert!(tags.contains(&"hybrid".to_string()));
    }

    #[test]
    fn archetype_tags_detect_secondary_mechanics() {
        let rs = run_with(&[
            CardId::Rupture,
            CardId::Offering,
            CardId::BattleTrance,
        ...
        let tags = CardEvaluator::archetype_tags(&CardEvaluator::deck_profile(&rs));
        assert!(tags.contains(&"self_damage".to_string()));
        assert!(tags.contains(&"draw_cycle".to_string()));
        assert!(tags.contains(&"power_scaling".to_string()));
        assert!(tags.contains(&"status".to_string()));
    }

    #[test]
    fn armaments_second_copy_is_heavily_discounted() {
        let first_copy_deck = run_with(&[CardId::Bash, CardId::ShrugItOff, CardId::Defend]);
        let second_copy_deck = run_with(&[
            CardId::Armaments,
            CardId::Bash,
            CardId::ShrugItOff,
            CardId::Defend,
        ...
        let first = CardEvaluator::evaluate_card(CardId::Armaments, &first_copy_deck);
        let second = CardEvaluator::evaluate_card(CardId::Armaments, &second_copy_deck);
    ...
    #[test]
    fn corruption_second_copy_is_heavily_discounted() {
        let first_copy_deck = run_with(&[
            CardId::TrueGrit,
            CardId::SecondWind,
            CardId::BurningPact,
            CardId::Defend,
        ...
        let first = CardEvaluator::evaluate_card(CardId::Corruption, &first_copy_deck);
        let second = CardEvaluator::evaluate_card(CardId::Corruption, &second_copy_deck);
    ...
    #[test]
    fn shrug_second_copy_is_reasonable_but_third_drops_more() {
        let second_copy_deck = run_with(&[CardId::ShrugItOff, CardId::Bash, CardId::Defend]);
        let third_copy_deck = run_with(&[
            CardId::ShrugItOff,
            CardId::ShrugItOff,
            CardId::Bash,
            CardId::Defend,
        ...
        let second = CardEvaluator::evaluate_card(CardId::ShrugItOff, &second_copy_deck);
        let third = CardEvaluator::evaluate_card(CardId::ShrugItOff, &third_copy_deck);
    ...
    #[test]
    fn second_limit_break_is_less_valuable_than_first_even_with_support() {
        let first_copy_deck = run_with(&[
            CardId::Inflame,
            CardId::HeavyBlade,
            CardId::TwinStrike,
        ...
        let first = CardEvaluator::evaluate_card(CardId::LimitBreak, &first_copy_deck);
        let second = CardEvaluator::evaluate_card(CardId::LimitBreak, &second_copy_deck);

        assert!(first > second);
    }

    #[test]
    fn strike_is_worse_than_engine_card_in_block_shell_for_purge() {
        let rs = run_with(&[
            CardId::Strike,
            CardId::Barricade,
            CardId::BodySlam,
        ...
        let strike = CardEvaluator::evaluate_owned_card(CardId::Strike, &rs);
        let barricade = CardEvaluator::evaluate_owned_card(CardId::Barricade, &rs);

        assert!(strike < barricade);
    }
}
