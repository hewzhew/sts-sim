...
fn exact_visible_belief(monster: &MonsterEntity) -> MonsterBeliefState {
    ...
    let preview = monster.move_preview();
    let (base_damage, hits) = attack_profile_from_spec(&plan.spec);
    let total_damage = preview.total_damage.unwrap_or(0);
    MonsterBeliefState {
        entity_id: monster.id,
        ...
        attack_probability: if preview.damage_per_hit.is_some() {
            1.0
        } else {
            0.0
        },
...
fn attack_profile_from_spec(spec: &MonsterMoveSpec) -> (i32, u8) {
    spec.attack()
