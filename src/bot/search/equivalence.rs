...
#[cfg(test)]
mod tests {
    use super::{reduce_search_moves, SearchEquivalenceKind, SearchEquivalenceMode};
    use crate::bot::search::profile::{
        SearchProfileCollector, SearchProfilePhase, SearchProfilingLevel,
    };
    ...
    fn reduced_inputs(
        ...
        mode: SearchEquivalenceMode,
    ) -> Vec<super::ReducedSearchMove> {
        let legal_moves =
            crate::bot::search::get_legal_moves(&EngineState::CombatPlayerTurn, combat);
    ...
    #[test]
    fn safe_collapses_repeated_pure_defends() {
        ...
        .with_energy(2);
        let reduced = reduced_inputs(&combat, SearchEquivalenceMode::Safe);
        let defend_clusters = reduced
            .iter()
            .filter(|entry| matches!(entry.input, ClientInput::PlayCard { .. }))
            .collect::<Vec<_>>();
        assert_eq!(defend_clusters.len(), 1, "{:?}", reduced);
        assert_eq!(defend_clusters[0].cluster_size, 2);
        assert_eq!(
            defend_clusters[0].equivalence_kind,
            Some(SearchEquivalenceKind::Exact)
        );
    }

    #[test]
    fn safe_collapses_same_target_vanilla_attacks() {
        let combat = combat_with_hand_and_intent(
            vec![
        ...
        let reduced = reduced_inputs(&combat, SearchEquivalenceMode::Safe);
        let attack_clusters = reduced
            .iter()
            .filter(|entry| matches!(entry.input, ClientInput::PlayCard { .. }))
            .collect::<Vec<_>>();
        assert_eq!(attack_clusters.len(), 1, "{:?}", reduced);
        assert_eq!(attack_clusters[0].cluster_size, 2);
    }

    #[test]
    fn safe_does_not_collapse_different_targets() {
        let combat =
            combat_with_hand_and_intent(vec![CombatCard::new(CardId::Strike, 1)], Intent::Buff)
                .with_monsters({
                    let mut monsters = combat_with_hand_and_intent(vec![], Intent::Buff)
                        .entities
                        .monsters;
        ...
        let attack_clusters = reduced
            ...
            .filter(|entry| matches!(entry.input, ClientInput::PlayCard { .. }))
            .collect::<Vec<_>>();
        assert_eq!(attack_clusters.len(), 2, "{:?}", reduced);
    }

    #[test]
    fn safe_does_not_collapse_draw_before_attack() {
        let combat = combat_with_hand_and_intent(
            vec![
    ...
    #[test]
    fn safe_does_not_collapse_setup_before_exhaust_outlet() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::DarkEmbrace, 1),
                CombatCard::new(CardId::TrueGrit, 2),
            ],
            Intent::Buff,
        );
        let reduced = reduced_inputs(&combat, SearchEquivalenceMode::Safe);
    ...
    #[test]
    fn safe_does_not_collapse_kill_sensitive_attacks() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::Strike, 1),
                CombatCard::new(CardId::Strike, 2),
        ...
        .with_monster_hp(1, 6)
        .with_monster_max_hp(1, 6);
        let reduced = reduced_inputs(&combat, SearchEquivalenceMode::Safe);
        let strikes = reduced
            .iter()
            .filter(|entry| matches!(entry.input, ClientInput::PlayCard { .. }))
            .collect::<Vec<_>>();
        assert_eq!(strikes.len(), 2, "{:?}", reduced);
    }

    #[test]
    fn experimental_can_merge_semantically_equal_non_identical_attacks() {
        let mut swift = CombatCard::new(CardId::SwiftStrike, 2);
        swift.base_damage_override = Some(6);
        swift.cost_for_turn = Some(1);
        let combat = combat_with_hand_and_intent(
            vec![CombatCard::new(CardId::Strike, 1), swift],
            Intent::Buff,
        );

        let safe = reduced_inputs(&combat, SearchEquivalenceMode::Safe);
        let experimental = reduced_inputs(&combat, SearchEquivalenceMode::Experimental);
        ...
        let experimental_attacks = experimental
            .iter()
            .filter(|entry| matches!(entry.input, ClientInput::PlayCard { .. }))
            .collect::<Vec<_>>();
        assert_eq!(experimental_attacks.len(), 1, "{:?}", experimental);
        assert_eq!(
            experimental_attacks[0].equivalence_kind,
            Some(SearchEquivalenceKind::Heuristic)
        );
    }
}
