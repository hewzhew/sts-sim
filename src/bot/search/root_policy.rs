...
#[cfg(test)]
mod tests {
    use super::{action_semantic_tags, motif_transition_bonus, StatePressureFeatures};
    ...
    #[test]
    fn visible_intent_pressure_prefers_visible_exact_and_does_not_auto_urgent_on_elite() {
        let combat = combat_with_hand_and_intent(vec![], Intent::Attack { damage: 6, hits: 1 })
            .with_player_hp(40)
            .with_elite_fight(true);
        let pressure = StatePressureFeatures::from_combat(&combat);
        assert_eq!(pressure.visible_incoming, 6);
        assert_eq!(pressure.value_incoming, 6);
        assert_eq!(pressure.survival_guard_incoming, 6);
        assert_eq!(pressure.visible_unblocked, 6);
        assert!(!pressure.lethal_pressure);
        assert!(!pressure.hidden_intent_active);
        assert!(!pressure.urgent_pressure);
        assert!(pressure.encounter_risk);
    }

    #[test]
    fn hidden_intent_pressure_uses_belief_expected_for_value_and_not_visible_exact() {
        use crate::content::monsters::EnemyId;
        use crate::content::relics::{RelicId, RelicState};

        let mut combat = combat_with_hand_and_intent(vec![], Intent::Attack { damage: 6, hits: 1 })
        ...
        assert!(pressure.hidden_intent_active);
        assert_eq!(pressure.visible_incoming, 6);
        assert_eq!(pressure.belief_expected_incoming, 0);
        assert_eq!(pressure.value_incoming, 0);
        assert_eq!(pressure.survival_guard_incoming, 0);
    }

    #[test]
    fn action_semantics_recognize_setup_attack_and_defensive_potion() {
        ...
        let mut potion_combat = combat.clone();
        potion_combat.entities.potions[0] = Some(Potion::new(PotionId::BlockPotion, 1));

        let setup = action_semantic_tags(
            &combat,
        ...
        let attack = action_semantic_tags(
            &combat,
            &ClientInput::PlayCard {
                card_index: 1,
                target: Some(1),
        ...
        let potion = action_semantic_tags(
            &potion_combat,
        ...
        assert!(setup.persistent_setup);
        assert!(!setup.attack_like);
        assert!(attack.attack_like);
        assert!(!attack.persistent_setup);
        assert!(potion.defensive_potion);
    }

    #[test]
    fn defensive_potion_bonus_is_positive_under_lethal_pressure() {
        ...
        next_combat.entities.player.block += 12;
        let bonus = motif_transition_bonus(
            &combat,
            &ClientInput::UsePotion {
                potion_index: 0,
                target: None,
