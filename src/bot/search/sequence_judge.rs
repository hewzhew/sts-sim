...
#[cfg(test)]
mod tests {
    use super::{SequenceCandidate, SequenceJudge};
    ...
    fn delta_for(adjustments: &[super::SequenceAdjustment], input: &ClientInput) -> f32 {
        adjustments
            .iter()
            .find(|adjustment| adjustment.input == *input)
            .map(|adjustment| adjustment.total_delta)
            .unwrap_or(0.0)
    }

    #[test]
    fn survive_now_template_penalizes_non_surviving_line_under_lethal_pressure() {
        ...
        let strike = ClientInput::PlayCard {
            card_index: 1,
            target: Some(1),
        };
        let candidates = vec![
            ...
            SequenceCandidate {
                ...
                projected_hp: 4,
                projected_block: 0,
                projected_enemy_total: 14,
                projected_unblocked: 14,
                survives: false,
            },
        ];

        let adjustments = SequenceJudge::new(&combat, &candidates).adjustments();
        assert!(delta_for(&adjustments, &strike) < -1_000_000.0);
        assert!(delta_for(&adjustments, &defend) > delta_for(&adjustments, &strike));
    }

    #[test]
    fn potion_bridge_template_rewards_defensive_potion_when_other_lines_die() {
        ...
        let potion = ClientInput::UsePotion {
            potion_index: 0,
            target: None,
        };
        let strike = ClientInput::PlayCard {
        ...
        let candidates = vec![
            ...
            SequenceCandidate {
                ...
                projected_block: 0,
                projected_enemy_total: 14,
                projected_unblocked: 18,
                survives: false,
            },
        ];

        let adjustments = SequenceJudge::new(&combat, &candidates).adjustments();
        assert!(delta_for(&adjustments, &potion) > 1_000_000.0);
    }

    #[test]
    fn setup_then_survive_template_keeps_best_surviving_setup_line_viable() {
        let combat = combat_with_hand_and_intent(
            vec![
        ...
        .with_player_hp(10);
        let setup = ClientInput::PlayCard {
            card_index: 0,
            target: None,
        };
        let defend = ClientInput::PlayCard {
        ...
        let mut setup_after_combat = combat.clone();
        setup_after_combat.zones.hand.remove(0);
        setup_after_combat.turn.energy = 0;
        setup_after_combat.entities.player.block = 10;
        let candidates = vec![
            SequenceCandidate {
                input: setup.clone(),
                after_engine: EngineState::CombatPlayerTurn,
                after_combat: setup_after_combat,
                passive: true,
                projected_hp: 10,
                projected_block: 0,
                projected_enemy_total: 20,
            ...
            SequenceCandidate {
                ...
                projected_block: 5,
                projected_enemy_total: 20,
                projected_unblocked: 6,
                survives: false,
            },
        ];

        let adjustments = SequenceJudge::new(&combat, &candidates).adjustments();
        assert!(delta_for(&adjustments, &setup) >= 0.0);
    }

    #[test]
    fn defer_setup_template_penalizes_playing_setup_before_safe_window() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::FeelNoPain, 1),
                CombatCard::new(CardId::Defend, 2),
        ...
        let mut play_now_after = combat.clone();
        play_now_after.zones.hand.remove(0);
        play_now_after.turn.energy = 0;

        let mut defer_after = combat.clone();
        defer_after.zones.hand.remove(1);
        defer_after.turn.energy = 1;
        defer_after.entities.player.block = 5;

        let candidates = vec![
            SequenceCandidate {
                input: setup.clone(),
                after_engine: EngineState::CombatPlayerTurn,
                after_combat: play_now_after,
                passive: true,
                projected_hp: 50,
                projected_block: 0,
                projected_enemy_total: 30,
                projected_unblocked: 10,
                survives: true,
            },
            SequenceCandidate {
                input: defend,
                after_engine: EngineState::CombatPlayerTurn,
                after_combat: defer_after,
                passive: false,
                projected_hp: 50,
                projected_block: 5,
                projected_enemy_total: 30,
                projected_unblocked: 5,
                survives: true,
            },
        ];
    ...
    #[test]
    fn defer_setup_template_is_not_million_scale_under_light_pressure() {
        let combat = combat_with_hand_and_intent(
            vec![
        ...
        let mut setup_after = combat.clone();
        setup_after.zones.hand.remove(0);
        setup_after.turn.energy = 2;

        let mut strike_after = combat.clone();
        strike_after.zones.hand.remove(1);
        strike_after.turn.energy = 1;

        let candidates = vec![
            SequenceCandidate {
                input: setup.clone(),
                after_engine: EngineState::CombatPlayerTurn,
                after_combat: setup_after,
                passive: true,
                projected_hp: 54,
                projected_block: 0,
                projected_enemy_total: 30,
            ...
            SequenceCandidate {
                input: strike,
                after_engine: EngineState::CombatPlayerTurn,
                after_combat: strike_after,
                passive: false,
                projected_hp: 54,
                projected_block: 0,
                projected_enemy_total: 24,
                projected_unblocked: 6,
                survives: true,
            },
        ];
        ...
        let adjustments = SequenceJudge::new(&combat, &candidates).adjustments();
        assert!(delta_for(&adjustments, &setup) > -100_000.0);
    }

    #[test]
    fn mild_opening_pressure_does_not_apply_million_scale_attack_penalty() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::Defend, 1),
                CombatCard::new(CardId::Strike, 2),
        ...
        let mut defend_after = combat.clone();
        defend_after.zones.hand.remove(0);
        defend_after.turn.energy = 0;
        defend_after.entities.player.block = 5;

        let mut strike_after = combat.clone();
        strike_after.zones.hand.remove(1);
        strike_after.turn.energy = 0;

        let candidates = vec![
            SequenceCandidate {
                input: defend,
                after_engine: EngineState::CombatPlayerTurn,
                after_combat: defend_after,
                passive: true,
                projected_hp: 72,
                projected_block: 5,
                projected_enemy_total: 28,
                projected_unblocked: 2,
                survives: true,
            },
            SequenceCandidate {
                input: strike.clone(),
                ...
                projected_hp: 72,
                projected_block: 0,
                projected_enemy_total: 22,
                projected_unblocked: 7,
                survives: true,
            },
        ];

        let adjustments = SequenceJudge::new(&combat, &candidates).adjustments();
        assert!(delta_for(&adjustments, &strike) > -100_000.0);
    }

    #[test]
    fn next_turn_setup_window_requires_original_setup_card_to_be_available() {
        let combat =
            combat_with_hand_and_intent(vec![CombatCard::new(CardId::FeelNoPain, 1)], Intent::Buff)
                .with_draw_ids(&[
                    CardId::Strike,
                    CardId::Strike,
                    CardId::Strike,
        ...
        let setup = super::setup_card_ref(
            &combat,
        ...
        .expect("setup card ref");

        let outcome = super::next_turn_setup_window_outcome(
    ...
    #[test]
    fn next_turn_setup_window_accepts_retained_setup_card() {
        let mut retained_setup = CombatCard::new(CardId::FeelNoPain, 1);
        retained_setup.retain_override = Some(true);
        let combat =
            combat_with_hand_and_intent(vec![retained_setup], Intent::Buff).with_draw_ids(&[
                CardId::Strike,
                CardId::Strike,
                CardId::Strike,
                CardId::Strike,
    ...
    #[test]
    fn followup_sequence_tracks_realized_exhaust_payoff() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::BurningPact, 1),
                CombatCard::new(CardId::Wound, 2),
            ],
            Intent::Buff,
        )
        .with_player_power(PowerId::FeelNoPain, 4)
        .with_player_power(PowerId::DarkEmbrace, 1)
        .with_draw_ids(&[CardId::Strike, CardId::Strike]);

        let outcome =
            super::evaluate_followup_sequence(&EngineState::CombatPlayerTurn, &combat, 2, 3, 80);

        assert!(outcome.realized_exhaust_block >= 4, "{:?}", outcome);
        assert!(outcome.realized_exhaust_draw >= 1, "{:?}", outcome);
    }
}
