...
mod tests {
    use super::{
        diagnose_root_search_with_depth_and_mode, find_best_move, find_best_move_with_mode,
        SearchEquivalenceMode,
    };
    ...
    #[test]
    fn guided_search_prefers_survival_card_over_setup_power() {
        let combat = combat_with_hand_and_intent(
            vec![
                crate::combat::CombatCard::new(CardId::Impervious, 1),
                crate::combat::CombatCard::new(CardId::Strike, 2),
            ],
            Intent::Attack {
                damage: 27,
                hits: 1,
        ...
        let choice = find_best_move(
            &EngineState::CombatPlayerTurn,
            &combat,
            5,
            false,
            &crate::bot::coverage::CoverageDb::load_or_default(),
            crate::bot::coverage::CoverageMode::Off,
            None,
        );

        let chosen_id = match choice {
            ClientInput::PlayCard { card_index, .. } => combat.zones.hand[card_index].id,
            _ => panic!("expected card play, got {:?}", choice),
        };
        assert_eq!(chosen_id, CardId::Impervious);
    }

    #[test]
    fn guided_search_avoids_opening_end_turn_when_active_line_exists() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::Defend, 1),
                CombatCard::new(CardId::Strike, 2),
                CombatCard::new(CardId::Defend, 3),
                CombatCard::new(CardId::FireBreathing, 4),
                CombatCard::new(CardId::Strike, 5),
            ],
            Intent::Attack { damage: 7, hits: 1 },
        )
        .with_monster_type(1, crate::content::monsters::EnemyId::LouseDefensive)
        .with_monster_max_hp(1, 16)
        .with_monster_hp(1, 16)
        .with_monsters({
            let mut monsters =
                combat_with_hand_and_intent(vec![], Intent::Attack { damage: 7, hits: 1 })
                    .with_monster_type(1, crate::content::monsters::EnemyId::LouseDefensive)
                    .with_monster_max_hp(1, 16)
                    .with_monster_hp(1, 16)
                    .entities
                    .monsters;
            monsters[0].current_intent = Intent::Attack { damage: 7, hits: 1 };
            monsters[0].intent_dmg = 7;
            let mut second = monsters[0].clone();
            second.id = 2;
            second.monster_type = crate::content::monsters::EnemyId::LouseNormal as usize;
            second.current_hp = 10;
            second.max_hp = 10;
            second.current_intent = Intent::Attack { damage: 6, hits: 1 };
            second.intent_dmg = 6;
            second.slot = 1;
        ...
        assert!(!matches!(choice, ClientInput::EndTurn));
        match choice {
            ClientInput::PlayCard { card_index, .. } => {
                assert_ne!(combat.zones.hand[card_index].id, CardId::FireBreathing);
            }
            other => panic!("expected actionable play, got {:?}", other),
        }
    }

    #[test]
    fn guided_search_keeps_passive_setup_below_survival_line() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::DarkEmbrace, 1),
        ...
        match choice {
            ClientInput::PlayCard { card_index, .. } => {
                assert_ne!(combat.zones.hand[card_index].id, CardId::DarkEmbrace);
            }
            other => panic!("expected card play, got {:?}", other),
        }
    }

    #[test]
    fn guided_search_does_not_play_bloodletting_without_cashout() {
        let combat = combat_with_hand_and_intent(
            vec![CombatCard::new(CardId::Bloodletting, 1)],
            Intent::Buff,
        )
        .with_energy(1);
    ...
    #[test]
    fn guided_search_prefers_dark_embrace_before_true_grit_window() {
        let mut true_grit_plus = CombatCard::new(CardId::TrueGrit, 2);
        true_grit_plus.upgrades = 1;
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::DarkEmbrace, 1),
                true_grit_plus,
                CombatCard::new(CardId::Burn, 3),
        ...
        assert_eq!(chosen_id, CardId::DarkEmbrace);
    }

    #[test]
    fn guided_search_uses_block_potion_when_it_is_the_survival_bridge() {
        let mut combat = combat_with_hand_and_intent(
            vec![],
            Intent::Attack {
                damage: 18,
                hits: 1,
        ...
        assert!(
            ...
            "expected block potion bridge, got {:?}",
            choice
        );
    }

    #[test]
    fn guided_search_prefers_defend_over_attack_under_high_pressure() {
        let combat = combat_with_hand_and_intent(
            vec![
        ...
        assert_eq!(chosen_id, CardId::Defend);
    }

    #[test]
    fn guided_search_assigns_positive_sequencing_bonus_to_inflame_setup_line() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::Inflame, 1),
                CombatCard::new(CardId::SwordBoomerang, 2),
                CombatCard::new(CardId::TwinStrike, 3),
            ],
            Intent::Buff,
        )
        .with_energy(3)
        ...
        .with_monster_hp(1, 120)
        .with_monster_max_hp(1, 120);
        ...
        let inflame = diagnostics
            .top_moves
            .iter()
            .find(|stat| match &stat.input {
                ClientInput::PlayCard { card_index, .. } => {
                    combat.zones.hand[*card_index].id == CardId::Inflame
                }
                _ => false,
            })
            .expect("inflame diagnostic present");
        assert!(inflame.sequence_frontload_bonus > 0.0, "{inflame:?}");
        assert!(inflame.order_score > inflame.leaf_score, "{inflame:?}");
    }

    #[test]
    fn guided_search_frontloads_energy_bridge_before_high_cost_payoff() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::SeeingRed, 1),
                CombatCard::new(CardId::Bludgeon, 2),
                CombatCard::new(CardId::Strike, 3),
            ],
            Intent::Buff,
        )
        ...
        assert_eq!(chosen_id, CardId::SeeingRed);
    }

    #[test]
    fn guided_search_prefers_bash_before_multi_hit_payoff() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::Bash, 1),
                CombatCard::new(CardId::TwinStrike, 2),
                CombatCard::new(CardId::Anger, 3),
            ],
            Intent::Buff,
        )
        .with_energy(3)
        ...
        assert_eq!(chosen_id, CardId::Bash);
    }

    #[test]
    fn equivalence_reduction_shrinks_root_move_count_for_duplicate_cards() {
        let combat = combat_with_hand_and_intent(
            vec![
                CombatCard::new(CardId::Defend, 1),
                CombatCard::new(CardId::Defend, 2),
        ...
        let safe = diagnose_root_search_with_depth_and_mode(
            &EngineState::CombatPlayerTurn,
            &combat,
            &crate::bot::coverage::CoverageDb::load_or_default(),
            crate::bot::coverage::CoverageMode::Off,
        ...
        let off = diagnose_root_search_with_depth_and_mode(
            &EngineState::CombatPlayerTurn,
            &combat,
            &crate::bot::coverage::CoverageDb::load_or_default(),
            crate::bot::coverage::CoverageMode::Off,
            ...
            5,
            0,
            SearchEquivalenceMode::Off,
        );

        assert!(safe.reduced_legal_moves < safe.legal_moves, "{:?}", safe);
        assert_eq!(off.reduced_legal_moves, off.legal_moves);
        assert!(
            safe.top_moves.iter().any(|stat| stat.cluster_size > 1),
            "{:?}",
            safe.top_moves
        );
    }
    ...
    #[test]
    fn safe_equivalence_keeps_existing_setup_window_choice() {
        let mut true_grit_plus = CombatCard::new(CardId::TrueGrit, 2);
        true_grit_plus.upgrades = 1;
        let combat = combat_with_hand_and_intent(
            vec![
        ...
        let safe = find_best_move_with_mode(
            &EngineState::CombatPlayerTurn,
            &combat,
            SearchEquivalenceMode::Safe,
            5,
        ...
        let off = find_best_move_with_mode(
            &EngineState::CombatPlayerTurn,
            &combat,
            SearchEquivalenceMode::Off,
            5,
