...
mod tests {
    ...
    #[test]
    fn taxonomy_covers_representative_ironclad_roles() {
        let demon_form = taxonomy(CardId::DemonForm);
        assert!(demon_form.is_strength_enabler());
        assert!(demon_form.is_setup_power());
        assert!(demon_form.is_scaling_power());

        let second_wind = taxonomy(CardId::SecondWind);
        assert!(second_wind.is_exhaust_outlet());
        assert!(second_wind.is_block_core());
        assert!(second_wind.is_armaments_upgrade_priority());

        let burning_pact = taxonomy(CardId::BurningPact);
        assert!(burning_pact.is_exhaust_outlet());
        assert!(burning_pact.is_draw_core());
        assert!(!burning_pact.is_resource_conversion());

        let inflame = taxonomy(CardId::Inflame);
        assert!(inflame.is_strength_enabler());
        assert!(inflame.is_setup_power());
        assert!(!inflame.is_scaling_power());

        let panacea = taxonomy(CardId::Panacea);
        assert!(!panacea.is_panacea_self_combo());

        let flex = taxonomy(CardId::Flex);
        assert!(flex.is_panacea_self_combo());

        let whirlwind = taxonomy(CardId::Whirlwind);
        assert!(whirlwind.is_strength_payoff());
        assert!(whirlwind.is_multi_attack_payoff());
        assert!(whirlwind.is_attack_followup_priority());

        let trip = taxonomy(CardId::Trip);
        assert!(trip.is_vuln_enabler());

        let headbutt = taxonomy(CardId::Headbutt);
        assert!(headbutt.is_discard_retrieval());
    }
}
