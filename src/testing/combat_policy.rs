...
mod tests {
    ...
    use crate::testing::support::test_support::{basic_combat, combat_with_hand, CombatTestExt};

    #[test]
    fn state_feature_snapshot_exposes_core_pressure_fields() {
        let combat = combat_with_hand(&[CardId::Strike, CardId::Defend, CardId::Bash])
            .with_monster_intent(1, Intent::Attack { damage: 8, hits: 1 })
            .with_energy(3);
        let features = extract_state_features(&combat);

        assert!(features.contains_key("player_hp"));
        assert!(features.contains_key("incoming_damage"));
        assert!(features.contains_key("posture_immediate_survival_pressure"));
        assert!(features.contains_key("hand_role_core_keeper"));
    }

    #[test]
    fn bad_action_tags_mark_survival_override_playing_status() {
        let combat = combat_with_hand(&[CardId::Slimed, CardId::Strike, CardId::Defend])
            .with_monster_intent(
                1,
                Intent::Attack {
                    damage: 20,
        ...
        let decision = PolicyDecision {
            ...
            final_input_debug: "PlayCard { card_index: 0, target: None }".to_string(),
            source: "tactical_override".to_string(),
            confidence: None,
            fallback_used: false,
            tactical_reason: Some("survival override".to_string()),
            candidate_scores: Vec::new(),
        };

        let tags = flag_bad_action_tags(&combat, &decision);
        assert!(tags.contains(&"survival_override_played_status_or_curse".to_string()));
    }

    #[test]
    fn bad_action_tags_mark_low_pressure_potion_use() {
        ...
        .with_player_hp(80)
        .with_energy(3)
        .with_monster_type(1, crate::content::monsters::EnemyId::LouseNormal)
        .with_monster_max_hp(1, 13)
        .with_monster_hp(1, 13)
        .with_monster_intent(1, Intent::Attack { damage: 5, hits: 1 });
        let mut second = basic_combat()
            .entities
            .monsters
            .into_iter()
            .next()
            .expect("default monster missing");
        second.id = 2;
        second.slot = 1;
        second.logical_position = 1;
        second.monster_type = crate::content::monsters::EnemyId::LouseDefensive as usize;
        second.max_hp = 11;
        second.current_hp = 11;
        second.current_intent = Intent::Attack { damage: 5, hits: 1 };
        second.intent_dmg = 5;
        combat.entities.monsters.push(second);
        combat.entities.potions[0] = Some(crate::content::potions::Potion::new(
        ...
        let decision = PolicyDecision {
            ...
            final_input_debug: "UsePotion { potion_index: 0, target: None }".to_string(),
            source: "tactical_override".to_string(),
            confidence: None,
            fallback_used: false,
            tactical_reason: Some("potion survival: survival margin".to_string()),
            candidate_scores: Vec::new(),
        };

        let tags = flag_bad_action_tags(&combat, &decision);
        assert!(tags.contains(&"potion_used_on_low_pressure_turn".to_string()));
    }

    #[test]
    fn bad_action_tags_mark_power_through_without_incoming() {
        ...
        .with_player_power(crate::content::powers::PowerId::FeelNoPain, 4)
        .with_monster_type(1, crate::content::monsters::EnemyId::Lagavulin)
        ...
        let decision = PolicyDecision {
            ...
            final_input_debug: "PlayCard { card_index: 4, target: None }".to_string(),
            source: "heuristic".to_string(),
            confidence: Some(1.0),
            fallback_used: false,
            tactical_reason: None,
        ...
        let tags = flag_bad_action_tags(&combat, &decision);
        assert!(tags.contains(&"power_through_played_without_incoming".to_string()));
    }
}
