...
#[cfg(test)]
mod tests {
    use super::{compile_combat_start_spec, compile_combat_start_spec_with_seed, CombatStartSpec};
    use crate::testing::fixtures::author_spec::{AuthorCardEntry, AuthorCardSpec, AuthorRelicSpec};

    fn sample_spec(seed: u64) -> CombatStartSpec {
        CombatStartSpec {
            name: format!("sample_{seed}"),
            ...
            master_deck: vec![
                ...
                AuthorCardSpec::Simple("Defend_R".to_string()),
                AuthorCardSpec::Detailed(AuthorCardEntry {
                    id: "Disarm".to_string(),
                    upgrades: 0,
                    cost: None,
                    misc: None,
    ...
    #[test]
    fn same_seed_generates_same_opening_state() {
        let spec = sample_spec(7);
        let (engine_a, combat_a) = compile_combat_start_spec(&spec).expect("spec A");
        let (engine_b, combat_b) = compile_combat_start_spec(&spec).expect("spec B");
        assert_eq!(format!("{engine_a:?}"), format!("{engine_b:?}"));
        assert_eq!(combat_a.zones.hand, combat_b.zones.hand);
        assert_eq!(combat_a.zones.draw_pile, combat_b.zones.draw_pile);
        assert_eq!(combat_a.entities.monsters, combat_b.entities.monsters);
    }

    #[test]
    fn different_seed_changes_opening_distribution() {
        let (engine_a, combat_a) = compile_combat_start_spec(&sample_spec(7)).expect("spec A");
        let (engine_b, combat_b) = compile_combat_start_spec(&sample_spec(8)).expect("spec B");
        assert_eq!(format!("{engine_a:?}"), "CombatPlayerTurn");
        assert_eq!(format!("{engine_b:?}"), "CombatPlayerTurn");
        let same_hand = combat_a.zones.hand == combat_b.zones.hand;
        let same_draw_pile = combat_a.zones.draw_pile == combat_b.zones.draw_pile;
        assert!(
            !(same_hand && same_draw_pile),
            "different seeds should change opening hand or draw order"
        );
    }

    #[test]
    fn explicit_seed_override_changes_natural_start_distribution() {
        let spec = sample_spec(7);
        let (_, combat_a) =
            compile_combat_start_spec_with_seed(&spec, 101).expect("override seed A");
        let (_, combat_b) =
            compile_combat_start_spec_with_seed(&spec, 202).expect("override seed B");
        let same_hand = combat_a.zones.hand == combat_b.zones.hand;
        let same_draw_pile = combat_a.zones.draw_pile == combat_b.zones.draw_pile;
        assert!(
            !(same_hand && same_draw_pile),
            "seed override should change opening hand or draw order"
        );
    }
}
