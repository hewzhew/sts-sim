...
mod tests {
    ...
    #[test]
    fn projected_hexaghost_script_applies_weak_rounding() {
        let spec: CombatStartSpec = serde_json::from_str(include_str!(
            "../../data/boss_validation/hexaghost_v3/start_spec.json"
        ))
        .expect("parse hexaghost v3 start spec");
        let (engine_state, mut combat) =
            compile_combat_start_spec_with_seed(&spec, 2010).expect("compile start spec");
        let monster_id = combat.entities.monsters[0].id;
        combat.entities.monsters[0].next_move_byte = 1;
        combat.entities.monsters[0].current_intent = Intent::Attack { damage: 5, hits: 6 };
        ...
        let summary = project_hexaghost_future_script(&engine_state, &combat).expect("projection");
        assert_eq!(summary.windows[0].move_kind, "Divider");
        assert_eq!(summary.windows[0].damage_per_hit, 1);
        assert_eq!(summary.windows[0].total_raw_damage, 6);
    }
}
