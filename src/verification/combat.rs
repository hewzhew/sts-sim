...
pub use crate::verification::diff::replay::{
...
pub fn build_combat_state_from_snapshots(
    ...
    crate::verification::diff::state_sync::build_combat_state_from_snapshots(
        truth_snapshot,
...
pub fn build_live_split_combat_snapshots_from_root(root: &Value) -> Result<(Value, Value), String> {
    crate::verification::diff::replay::build_live_split_combat_snapshots_from_root(root)
}
...
pub fn mapped_command_to_input(
    ...
    crate::verification::diff::replay::mapped_command_to_input(command, combat)
}
...
pub fn compare_combat_states_from_snapshots(
    ...
    crate::verification::diff::replay::compare_states_from_snapshots(
        expected,
        truth_snapshot,
        observation_snapshot,
        was_end_turn,
...
pub fn load_live_session_replay(path: &std::path::Path) -> Result<LiveSessionReplay, String> {
    crate::verification::diff::replay::load_live_session_replay_path(path)
}

pub fn derive_replay_view(session: &LiveSessionReplay) -> CombatReplayView {
    crate::verification::diff::replay::derive_combat_replay_view(session)
}
...
pub fn verify_replay_view(
    ...
    crate::verification::diff::replay::verify_combat_replay_view(view, fail_fast)
}

pub fn build_live_session_replay_from_raw(
    path: &std::path::Path,
) -> Result<LiveSessionReplay, String> {
    crate::verification::diff::replay::build_live_session_replay_from_raw_path(path)
}
...
pub fn write_live_session_replay(
    ...
    crate::verification::diff::replay::write_live_session_replay_to_path(replay, path)
}
...
pub fn generate_live_session_replay(
    ...
    crate::verification::diff::replay::generate_live_session_replay_sidecar(raw_path, replay_path)
}
